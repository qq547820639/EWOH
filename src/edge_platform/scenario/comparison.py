...
class PlanComparison:
    ...
    def to_dict(self):
        return {
            "plans": [p.to_dict() if hasattr(p, "to_dict") else p
                      for p in self.plans],
...
def _build_recommendation(plans, metric_table):
    ...
    if cap is not None:
        ...
        if baseline_output > 1e-9:
            ...
            parts.append(
                "产能优先方案节拍提升 %.1f%% 但高负荷人员 %d 人、低电量风险 %d 台；" % (
                    uplift, cap_high, cap_low))
        else:
            parts.append(
                "产能优先方案预计产量 %.1f 但高负荷人员 %d 人、低电量风险 %d 台；" % (
                    cap_out, cap_high, cap_low))
    ...
    if safe is not None:
        ...
        if baseline_output > 1e-9:
            ...
            parts.append(
                "负荷均衡方案节拍提升 %.1f%% 但高负荷人员 %d 人、低电量风险 %d 台。" % (
                    uplift, safe_high, safe_low))
        else:
            parts.append(
                "负荷均衡方案预计产量 %.1f 但高负荷人员 %d 人、低电量风险 %d 台。" % (
                    safe_out, safe_high, safe_low))

    # 推荐逻辑：首期安全优先——若负荷均衡方案高负荷人员为 0，倾向推荐；
    # 否则推荐产量最高方案（均需班组长确认，spec「人在回路」）。
    if (safe is not None
            and val(getattr(safe, "plan_id", ""), "high_load_persons_count") == 0):
        parts.append("综合安全与负荷均衡考虑，建议优先采用负荷均衡方案；"
                     "如产能压力较大可由班组长确认后改用产能优先方案。")
    elif cap is not None:
        parts.append("建议采用产能优先方案，但需关注高负荷人员与低电量设备风险，"
                     "由班组长确认后执行。")
    return "".join(parts)
...
def format_comparison_table(comparison):
    ...
    headers = ["指标"] + [getattr(p, "plan_type", getattr(p, "plan_id", ""))
                        for p in plans]
    lines = []
