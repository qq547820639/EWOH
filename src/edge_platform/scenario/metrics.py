...
def compute_metrics(plan_assignment, ctx):
    ...
    for person_id, assignment in (plan_assignment or {}).items():
        ...
        predicted_duration = (float(tstate.get("predicted_duration_min", 0.0) or 0.0)
                              / max(speed_factor, 1e-9))
        ...
        if (cur.get("station_id") != station_id
                or cur.get("task_id") != task_id
                or cur.get("device_id") != device_id):
            affected_persons.append(person_id)

        # 工位新占用人数计数（用于拥堵变化）
        per_station_new_occupancy[station_id] = (
            per_station_new_occupancy.get(station_id, 0) + 1)
    ...
    low_battery_devices = []
    assigned_device_set = {a.get("device_id", "")
                           for a in (plan_assignment or {}).values()
                           if a and a.get("device_id")}
    ...
    for station_id, new_occ in per_station_new_occupancy.items():
        cur_occ = int((stations_state.get(station_id, {}) or {}).get(
            "current_occupancy", 0) or 0)
        congestion_delta += (new_occ - cur_occ)

    # 聚合
    on_time_probability = (sum(on_time_probs) / len(on_time_probs)
                           if on_time_probs else 1.0)
    ...
    assumptions["高负荷判定"] = "当前负荷 + 任务负荷增量 > 阈值 %.2f" % load_high_threshold
    assumptions["低电量判定"] = ("预测电量=当前电量−消耗率×剩余小时 < 阈值 %.2f；"
                              "故障设备直接标记" % battery_low_threshold)
