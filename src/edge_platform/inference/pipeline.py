...
_KEY_CHANNELS = ("pitch_deg", "roll_deg", "torque_nm", "assist_level",
...
class InferencePipeline:
    def __init__(self, storage, bus, registry, rules, event_engine=None,
                 firmware_whitelist=None, metrics_collector=None,
        ...
        self._firmware_whitelist = (set(firmware_whitelist)
                                    if firmware_whitelist else None)
    ...
    def _rule_label(feats):
        ...
        is_walk = (feats["gyro_mag_mean"] > 40
                   or feats["accel_mag_std"] > 1.5)
        is_high_load = (feats["torque_mean"] > 15
                        or feats["assist_mean"] > 0.5)
    ...
    def _check_unknown_triggers(self, window, feats, label, conf, model, is_rule,
                                firmware_version):
    ...
    def _check_consent(self, msg):
        ...
        try:
            allowed = self._consent_manager.is_allowed(
                person_id, CONSENT_PURPOSE_TELEMETRY)
        except Exception:
    ...
    def _infer(self, dev, window):
        ...
        if label != "unknown":
            triggered = self._check_unknown_triggers(
                window, feats, label, conf, model, is_rule, firmware_version)
            if triggered:
                label = "unknown"
                reason = triggered
        res = {
