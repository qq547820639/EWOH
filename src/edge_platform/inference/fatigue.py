...
def _weighted_recent_load(samples: List[LoadSample], now_ms: int,
                          window_ms: int) -> float:
...
def _load_slope_per_min(samples: List[LoadSample], now_ms: int,
                        window_ms: int) -> float:
    """近期负荷相对时间的线性回归斜率（负荷/分钟，正=上升）。"""
    items = [s for s in samples
             if 0 <= now_ms - ts_to_ms(s.ts) <= window_ms]
    if len(items) < 2:
        return 0.0
...
def _assert_non_medical(report: FatigueReport) -> None:
    ...
    for term in FORBIDDEN_MEDICAL_TERMS:
        assert term not in blob, "负荷趋势评分出现禁止的医学表述：%r" % term
...
class FatigueScorer:
    ...
    def __init__(self, buffer_sec: int = DEFAULT_BUFFER_SEC,
    ...
    def update_baseline(self, person_id: str,
                        shift_load_samples: List[LoadSample]) -> float:
        """根据参考班次样本计算个体基线。
    ...
    def score(self, person_id: str, horizon_min: int = 30) -> FatigueReport:
        ...
        assist_benefit = sum(max(0.0, s.assist_level) * max(0.0, s.duration_sec)
                             for s in recent)
        ...
        if not samples:
            ...
            recommendation = ("当前任务结束后安排低负荷工位或短时恢复"
                               "（建议恢复 %d 分钟）" % rec_min)
        elif level is LoadLevel.MEDIUM:
            if trend is TrendDirection.RISING:
                recommendation = ("负荷上升，建议适时安排短时恢复"
                                   "（建议恢复 %d 分钟）" % rec_min)
            else:
        ...
        # ---- 置信度（样本覆盖 × 新鲜度） ----
        recent_count = sum(1 for s in samples
                           if 0 <= now_ms - ts_to_ms(s.ts) <= horizon_ms)
...
def format_report(report: FatigueReport) -> str:
    ...
    """
    level_zh = {"LOW": "低", "MEDIUM": "中", "HIGH": "高"}.get(
        report.current_load_level, report.current_load_level)
    trend_zh = {"RISING": "上升", "STABLE": "平稳", "FALLING": "下降"}.get(
        report.trend_30min, report.trend_30min)
    causes_text = "、".join(report.main_causes) if report.main_causes else "无明显异常因素"
    lines = [
        "当前负荷：%s" % level_zh,
        "30分钟趋势：%s" % trend_zh,
        "主要原因：%s" % causes_text,
        "建议：%s" % report.recommendation,
    ]
    return "\n".join(lines)
