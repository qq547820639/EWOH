...
class _BasePurgeTest(unittest.TestCase):
    ...
    def _insert_telemetry(self, record_id, ts):
        self.storage.insert_telemetry({
            "record_id": record_id, "device_id": "EXO-1", "timestamp": ts,
            "sequence": 0, "source_type": "simulated",
            "telemetry": {"pitch_deg": 10.0, "torque_nm": 5.0},
            "quality": {"status": "good"}})

    def _insert_inference(self, inference_id, ts):
        self.storage.insert_inference({
            "inference_id": inference_id, "device_id": "EXO-1",
            "ts_start": ts, "ts_end": ts, "label": "stand",
            "confidence": 0.9, "model_id": "m1", "model_version": "0.1",
            "source_type": "simulated", "meta": {}})

    def _insert_event(self, event_id, ts, status="open"):
        self.storage.insert_event({
            "event_id": event_id, "event_code": "LOAD_CONTINUOUS",
            "severity": "L2", "status": status, "person_id": "P-1",
            "device_id": "EXO-1", "start_time": ts,
            "trigger": {"type": "rule", "rule_version": "v1"},
            "evidence": {"window_before_sec": 30}, "source_type": "simulated"})
    ...
    def _insert_audit_log_raw(self, audit_id, ts):
        ...
        with self.storage._lock, self.storage._db:
            self.storage._db.execute(
                "INSERT INTO audit_log (audit_id, action, actor_id, target_type,"
                " target_id, ts) VALUES (?,?,?,?,?,?)",
...
class HighFreqTelemetryPurgeTest(_BasePurgeTest):
    def test_purge_old_keeps_new(self):
        ...
        deleted = self.executor.purge_expired(
            self.storage, DataClass.HIGH_FREQ_TELEMETRY)
        self.assertEqual(deleted, 1)
        remaining = [r["record_id"] for r in self.storage._db.execute(
            "SELECT record_id FROM telemetry").fetchall()]
...
class MinuteAggPurgeTest(_BasePurgeTest):
    def test_purge_old_inference(self):
        ...
        self.assertEqual(deleted, 1)
        remaining = [r["inference_id"] for r in self.storage._db.execute(
            "SELECT inference_id FROM inference").fetchall()]
...
class EventEvidencePurgeTest(_BasePurgeTest):
    def test_only_closed_expired_purged(self):
        ...
        self.assertEqual(deleted, 1)
        remaining = [r["event_id"] for r in self.storage._db.execute(
            "SELECT event_id FROM risk_event").fetchall()]
...
class AuditLogPurgeTest(_BasePurgeTest):
    def test_audit_log_180_day_floor(self):
        ...
        self.assertEqual(deleted, 1)
        remaining = [r["audit_id"] for r in self.storage._db.execute(
            "SELECT audit_id FROM audit_log WHERE action!='purge'").fetchall()]
    ...
    def test_audit_log_floor_enforced_even_if_policy_smaller(self):
        ...
        self.rm.register(RetentionPolicy(
            data_class=DataClass.AUDIT_LOG, retention_days=30, note="违规改小"))
...
class PurgeAllTest(_BasePurgeTest):
    ...
    def test_purge_all_accepts_string_data_class(self):
        ...
        self._insert_telemetry("TS-OLD", _ts_days_ago(40))
        deleted = self.executor.purge_expired(
            self.storage, "HIGH_FREQ_TELEMETRY")
        self.assertEqual(deleted, 1)
...
class BatchSizeTest(_BasePurgeTest):
    def test_batched_delete_with_small_batch(self):
        ...
        deleted = self.executor.purge_expired(
            self.storage, DataClass.HIGH_FREQ_TELEMETRY, batch_size=2)
...
class CustomPolicyTest(_BasePurgeTest):
    def test_custom_shorter_policy_respected(self):
        ...
        self.rm.register(RetentionPolicy(
            data_class=DataClass.HIGH_FREQ_TELEMETRY, retention_days=7))
