...
class MetricsCollectorTest(unittest.TestCase):
    ...
    def test_set_device_stats(self):
        c = MetricsCollector()
        c.set_device_stats(online_count=5, offline_count=2,
                           avg_packet_loss_pct=3.5, low_battery_count=1)
        snap = c.snapshot()
...
class MonitoringRoutesTest(unittest.TestCase):
    def setUp(self):
        ...
        self.storage.insert_event({
            "event_id": "EVT-T1", "event_code": "LOAD_HIGH", "severity": "L2",
            "status": "open", "device_id": "EXO-003", "person_id": "P-003",
            "start_time": _evt_start,
            "trigger": {}, "evidence": {}, "source_type": "simulated"})
        self.storage.insert_inference({
            "inference_id": "INF-T1", "device_id": "EXO-001",
            "ts_start": "2026-07-31T10:00:00+08:00",
            "ts_end": "2026-07-31T10:00:01+08:00", "label": "stand",
            "source_type": "simulated"})
        self.storage.insert_inference({
            "inference_id": "INF-T2", "device_id": "EXO-001",
            "ts_start": "2026-07-31T10:00:01+08:00",
            "ts_end": "2026-07-31T10:00:02+08:00", "label": "unknown",
            "source_type": "simulated"})
        ...
        manager.start()
        self.ctx = server.Context(self.storage, bus=bus, pipeline=pipeline, registry=registry,
                                  rules=rules, manager=manager, metrics=self.metrics)
    ...
    def test_inference_metrics_route_returns_json(self):
        ...
        status, headers, body = self.harness.get(
            "/api/inference/metrics?start=2026-07-31T00:00:00%2B08:00"
            "&end=2026-08-01T00:00:00%2B08:00")
        self.assertEqual(status, 200)
    ...
    def test_event_status_close_records_close_duration(self):
        ...
        status, resp = self.harness.post("/api/event/status", {
            "event_id": "EVT-T1", "status": "closed",
            "handling": {"handled_by": "班组长A"}})
        self.assertEqual(status, 200)
    ...
    def test_recommend_and_confirm_update_adoption(self):
        ...
        status, resp = self.harness.post("/api/tasks/recommend", {
            "required_skill": "搬运", "zone_id": "月台A", "load_level": 0.3})
        self.assertEqual(status, 200)
        ...
        status2, resp2 = self.harness.post("/api/tasks/confirm", {
            "task_id": "T-MON-1", "person_id": "P-001", "confirmer": "班组长A",
            "required_skill": "搬运", "zone_id": "月台A"})
...
class PipelineIntegrationTest(unittest.TestCase):
    ...
    def test_stub_pipeline_accepts_collector(self):
        ...
        try:
            ...
            pipe = stubs.InferencePipeline(storage, bus, registry, rules,
