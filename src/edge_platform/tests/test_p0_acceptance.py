...
class P0Scenario1FullClosedLoopTest(unittest.TestCase):
    ...
    def test_device_online_to_event_close_full_loop(self):
        ...
        inj = WireInjector(device_id="EXO-P0-01", source_label="controlled_test",
                           hz=20.0, start_ts_ms=1_000_000, battery_pct=80)
        adapter = NyExoA1Adapter("EXO-P0-01", source_type="controlled_test",
                                 worker_id="P-001")
        ...
        # 3. 人工处置：确认事件
        self.storage.update_event_status(event_id, "confirmed",
                                         {"handled_by": "leader1", "action": "confirm",
                                          "handled_at": _iso(datetime.now()),
                                          "comment": "已现场核实"})
        if hasattr(self.storage, "insert_event_handling"):
            self.storage.insert_event_handling(event_id, "leader1", "confirm",
                                               comment="已现场核实")
        evt = self.storage.get_event(event_id)
        self.assertEqual(evt["status"], "confirmed")

        # 4. 关闭事件
        self.storage.update_event_status(event_id, "closed",
                                         {"handled_by": "leader1", "action": "close",
                                          "handled_at": _iso(datetime.now()),
                                          "comment": "已调整工位"})
        if hasattr(self.storage, "insert_event_handling"):
            self.storage.insert_event_handling(event_id, "leader1", "close",
                                               comment="已调整工位")
        evt = self.storage.get_event(event_id)
...
class P0Scenario2DisconnectBackfillTest(unittest.TestCase):
    ...
    def test_disconnect_reconnect_no_data_loss(self):
        inj = WireInjector(device_id="EXO-P0-02", source_label="controlled_test",
                           hz=20.0, start_ts_ms=1_000_000, battery_pct=80)
...
class P0Scenario3RuleDedupCooldownRecoverTest(unittest.TestCase):
    ...
    def test_rule_trigger_dedup_cooldown_and_auto_close(self):
        ...
        registry = stubs.ModelRegistry(Path(self.tmp) / "models")
        # 短窗规则：1s 触发，5s 冷却
        rules = RuleEngine("risk-rule-v1.0",
                           {"load_sec": 1, "cooldown_sec": 5})
        event_engine = EventEngine(self.storage, bus)
        pipeline = InferencePipeline(self.storage, bus, registry, rules,
                                     event_engine=event_engine)

        inj = WireInjector(device_id="EXO-P0-03", source_label="controlled_test",
                           hz=20.0, start_ts_ms=1_000_000, battery_pct=80)
        adapter = NyExoA1Adapter("EXO-P0-03", source_type="controlled_test",
                                 worker_id="P-001")
        ...
        load_events = [e for e in events_after_trigger
                       if e.get("event_code") == "LOAD_CONTINUOUS"]
        ...
        load_events_2 = [e for e in events_after_cooldown
                         if e.get("event_code") == "LOAD_CONTINUOUS"]
        self.assertEqual(len(load_events_2), 1,
                         "冷却期内不应重复开新事件（事件去重）")
...
class P0Scenario4ExportAuditTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.storage.insert_telemetry({
            "record_id": "TS-P0-001", "device_id": "EXO-001",
        ...
        cls.ctx = server.Context(cls.storage, bus=bus, pipeline=pipeline,
    ...
    def test_export_creates_audit_log(self):
        ...
        data = json.dumps({"device_id": "EXO-001", "start": s, "end": e,
                           "format": "json"}).encode()
        r = urllib.request.Request(
            self.base + "/api/telemetry/export", data=data, method="POST",
