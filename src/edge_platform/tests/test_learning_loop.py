...
class _LearningLoopTestBase(unittest.TestCase):
    ...
    def _adopted(self, loop, sched, actual_production, actual_body_load,
                 production_uplift=0.5, current_load=0.4):
...
class ComputeStatsTest(_LearningLoopTestBase):
    ...
    def test_compute_stats_improvement_rate_of_adopted(self):
        ...
        self.assertAlmostEqual(stats.improvement_rate_of_adopted,
                               2.0 / 3.0, places=9)
    ...
    def test_compute_stats_period_filtering(self):
        ...
        self._proposed_only(loop, sched)
        # 全包含窗口
        wide = loop.compute_stats(period_start="2000-01-01T00:00:00+00:00",
                                  period_end="2099-01-01T00:00:00+00:00")
        self.assertEqual(wide.total_requests, 2)
        # 全排除窗口（早于一切记录）
        empty = loop.compute_stats(period_start="2000-01-01T00:00:00+00:00",
                                   period_end="2000-01-02T00:00:00+00:00")
...
class SuggestCalibrationsTest(_LearningLoopTestBase):
    def test_suggest_calibrations_generates(self):
        ...
        self._adopted(loop, sched, actual_production=0.5, actual_body_load=0.6,
                      production_uplift=0.8, current_load=0.2)
    ...
    def test_suggest_calibrations_no_suggestion_without_deviation(self):
        ...
        self._adopted(loop, sched, actual_production=0.7, actual_body_load=0.2,
                      production_uplift=0.5, current_load=0.4)
        suggestions = loop.suggest_calibrations()
...
class ExportPeriodReportTest(_LearningLoopTestBase):
    def test_export_period_report_complete(self):
        ...
        self._adopted(loop, sched, actual_production=0.5, actual_body_load=0.6,
                      production_uplift=0.8, current_load=0.2)
        self._adopted(loop, sched, actual_production=0.4, actual_body_load=0.7,
                      production_uplift=0.8, current_load=0.2)
        self._proposed_only(loop, sched)
        ...
        # stats 完整
        stats = report["stats"]
        for key in ("total_requests", "proposed_count", "adopted_count",
                    "rejected_count", "adoption_rate",
                    "adopted_with_improvement", "improvement_rate_of_adopted",
                    "meets_target", "avg_metric_deltas",
