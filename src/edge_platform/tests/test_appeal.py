...
class SubmitTest(unittest.TestCase):
    def test_submit_success_fields_complete(self):
        ch = AppealChannel()
        rec = ch.submit(
            "p1", AppealType.MISJUDGEMENT, _target("recognition_result", "REC-1"),
            "动作识别为弯腰，实际为蹲下", evidence_refs=["clip-001", "witness-leader"],
        )
        ...
        # evidence_refs 默认空列表
        rec2 = ch.submit("p1", AppealType.SPECIAL_CIRCUMSTANCE, _target(),
                         "临时身体不适")
...
class LifecycleTest(unittest.TestCase):
    def test_acknowledge_state_transition(self):
        ch = AppealChannel()
        rec = ch.submit("p1", AppealType.SCHEDULING_OBJECTION,
                        _target("schedule_request", "REQ-1"), "负荷评分过高")
    ...
    def test_resolve_success_state_transition(self):
        ch = AppealChannel()
        rec = ch.submit("p1", AppealType.MISJUDGEMENT,
                        _target("recognition_result", "REC-1"), "识别错误")
...
class QueryTest(unittest.TestCase):
    def _seed(self):
        ch = AppealChannel()
        r1 = ch.submit("p1", AppealType.MISJUDGEMENT,
                       _target("recognition_result", "REC-1"), "识别错误")
        r2 = ch.submit("p1", AppealType.SCHEDULING_OBJECTION,
                       _target("schedule_request", "REQ-1"), "负荷过高")
        r3 = ch.submit("p2", AppealType.MISJUDGEMENT,
                       _target("recognition_result", "REC-1"), "我也是误判")
        r4 = ch.submit("p2", AppealType.SPECIAL_CIRCUMSTANCE,
                       _target("risk_event", "EV-9"), "身体不适")
...
class StatsTest(unittest.TestCase):
    def test_stats_counts(self):
        ch = AppealChannel()
        r1 = ch.submit("p1", AppealType.MISJUDGEMENT,
                       _target("recognition_result", "REC-1"), "误判1")
        r2 = ch.submit("p1", AppealType.MISJUDGEMENT,
                       _target("recognition_result", "REC-2"), "误判2")
        r3 = ch.submit("p2", AppealType.SCHEDULING_OBJECTION,
                       _target("schedule_request", "REQ-1"), "异议")
        ch.submit("p2", AppealType.DATA_DISPUTE,
                  _target("sensor", "S-1"), "争议")
        ...
        self.assertEqual(s.by_type, {
            "misjudgement": 2, "scheduling_objection": 1, "data_dispute": 1,
        })
...
class AuditLogTest(unittest.TestCase):
    def test_audit_log_complete(self):
        ...
        self.assertEqual(actions, [
            "submit", "acknowledge", "resolve", "submit", "reject",
        ])
        # 每条结构完整
        for e in log:
            self.assertIn("ts", e)
