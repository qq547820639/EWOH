...
class AuditLogTest(_BaseStorageTest):
    def test_insert_and_list(self):
        rec = self.storage.insert_audit_log(
            "create", "U-1", "risk_event", "EVT-1",
            before=None, after={"status": "open"}, result="success",
            request_id="REQ-1", source_ip="10.0.0.1")
        ...
        rec2 = self.storage.insert_audit_log(
            "update", "U-2", "device", "EXO-001",
            before={"online": 0}, after={"online": 1})
    ...
    def test_filter_by_action_actor_target(self):
        ...
        # 组合过滤
        self.assertEqual(
            len(self.storage.list_audit_logs(action="create", actor_id="U-1")), 1)
        # 不匹配返回空
        self.assertEqual(
            len(self.storage.list_audit_logs(target_type="person")), 0)
...
class DeviceProtocolVersionTest(_BaseStorageTest):
    def test_insert_and_list(self):
        r1 = self.storage.insert_device_protocol_version(
            "EXO-001", "v1.2", "fw-1.2.0", "hw-A")
        r2 = self.storage.insert_device_protocol_version(
            "EXO-001", "v1.3", "fw-1.3.0", "hw-A", audit_ref="AUD-X")
        self.storage.insert_device_protocol_version(
            "EXO-002", "v1.2", "fw-1.2.0", "hw-B")
...
class EventHandlingTest(_BaseStorageTest):
    def test_insert_and_list(self):
        r1 = self.storage.insert_event_handling(
            "EVT-1", "班组长A", "confirm", comment="已现场核实")
        r2 = self.storage.insert_event_handling(
            "EVT-1", "班组长A", "close", audit_ref="AUD-Y")
...
class AssignmentTest(_BaseStorageTest):
    def test_upsert_insert_then_update_id_stable(self):
        rec = self.storage.upsert_assignment(
            "ASG-001", "P-001", device_id="EXO-001", task_id="T-1",
            status="proposed", recommended_by="scheduler-1")
        ...
        rec2 = self.storage.upsert_assignment(
            "ASG-001", "P-001", device_id="EXO-001", task_id="T-1",
            status="confirmed", recommended_by="scheduler-1",
            confirmed_by="班组长A", confirmed_at="2026-07-31T10:00:00+08:00",
            audit_ref="AUD-C")
    ...
    def test_list_filters(self):
        ...
        self.assertEqual(len(self.storage.list_assignments(
            person_id="P-001", status="confirmed")), 0)
        # 组合命中
        self.assertEqual(len(self.storage.list_assignments(
            person_id="P-002", status="confirmed")), 1)


class ModelRegistryTableTest(_BaseStorageTest):
    def test_insert_and_list(self):
        r1 = self.storage.insert_model_record(
            "MODEL-A", "action_classifier", "0.1", model_card_uri="card://a")
        self.storage.insert_model_record(
            "MODEL-B", "action_classifier", "0.2", status="active")
        self.storage.insert_model_record(
            "MODEL-C", "fatigue_scorer", "0.1", status="retired")
        ...
        # 按 model_type
        self.assertEqual(
            len(self.storage.list_models(model_type="action_classifier")), 2)
        # 按 status
        ...
        # 组合
        self.assertEqual(len(self.storage.list_models(
            model_type="fatigue_scorer", status="retired")), 1)
        self.assertEqual(len(self.storage.list_models(
            model_type="action_classifier", status="retired")), 0)
...
class RuleRegistryTableTest(_BaseStorageTest):
    def test_insert_and_list(self):
        r1 = self.storage.insert_rule_record(
            "RULE-LOAD", "v0.1", enabled=True,
            config_json={"threshold": 0.7}, severity="L2", approver_id="admin1")
        r2 = self.storage.insert_rule_record(
            "RULE-POSTURE", "v0.1", enabled=False, severity="L1")
        self.storage.insert_rule_record(
            "RULE-LOAD", "v0.2", enabled=True, config_json={"threshold": 0.8})
...
class ConsentRecordTableTest(_BaseStorageTest):
    def test_insert_and_list(self):
        r1 = self.storage.insert_consent_record(
            "CONSENT-1", "P-001", "TELEMETRY", "leader1")
        self.storage.insert_consent_record(
            "CONSENT-2", "P-001", "VIDEO", "leader1", status="active")
        r3 = self.storage.insert_consent_record(
            "CONSENT-3", "P-002", "TELEMETRY", "leader2",
            status="revoked", revoked_at="2026-07-31T12:00:00+08:00",
            revoke_reason="员工离职", audit_ref="AUD-R")
        ...
        # 组合
        self.assertEqual(len(self.storage.list_consent_records(
            person_id="P-002", status="revoked")), 1)
        self.assertEqual(len(self.storage.list_consent_records(
            person_id="P-001", status="revoked")), 0)
...
class MigrationTest(unittest.TestCase):
    ...
    def test_v001_upgrade_creates_all_tables(self):
        ...
        for t in ("device_protocol_version", "event_handling", "assignment",
                  "model_registry", "rule_registry", "consent_record", "audit_log"):
...
class OldTablesUnaffectedTest(_BaseStorageTest):
    ...
    def test_old_tables_crud_and_counts(self):
        ...
        self.storage.insert_telemetry({
            "record_id": "TS-1", "device_id": "EXO-1", "timestamp": "2026-07-31T00:00:00+08:00",
            "sequence": 1, "source_type": "simulated", "telemetry": {"x": 1},
            "quality": {"status": "good"}})
        self.storage.insert_inference({
            "inference_id": "INF-1", "device_id": "EXO-1", "ts_start": "2026-07-31T00:00:00+08:00",
            "ts_end": "2026-07-31T00:00:00+08:00", "label": "stand", "source_type": "simulated"})
        self.storage.insert_event({
            "event_id": "EVT-1", "event_code": "X", "severity": "L1", "status": "open",
            "start_time": "2026-07-31T00:00:00+08:00", "trigger": {}, "evidence": {},
