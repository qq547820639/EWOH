...
from edge_platform.scheduler import (
    ...
    ScoringWeights, WeightAuditLog, Scorer,
...
class HardConstraintsTest(unittest.TestCase):
    ...
    def test_multiple_constraint_types(self):
        constraints = HardConstraints(
            ...
            shift_rules={"shiftA": {"rest_minutes_per_hour": 10,
        ...
        task = {
            "task_id": "t1", "required_skills": {"lifting"}, "station_id": "s1",
            "zone_id": "z_forbidden", "load_level": "HIGH", "action_type": "carry",
...
class ScorerTest(unittest.TestCase):
    def test_weights_applied_and_breakdown_sums_to_total(self):
        weights = ScoringWeights(
            w1_production=2.0, w2_on_time=1.0, w3_safety_risk=1.0,
            w4_body_load=1.0, w5_travel_distance=0.1, w6_changeover_cost=1.0,
        )
        ...
        expected = (2.0 * 0.5 + 1.0 * 0.8 - 1.0 * 0.3
                    - 1.0 * 0.6 - 0.1 * 10.0 - 1.0 * 1.0)
        self.assertAlmostEqual(total, expected, places=9)
        # 分项加权贡献之和 == 总分
        contrib_sum = (bd["w1_production_contrib"] + bd["w2_on_time_contrib"]
                       + bd["w3_safety_contrib"] + bd["w4_body_load_contrib"]
                       + bd["w5_travel_contrib"] + bd["w6_changeover_contrib"])
...
class WeightAuditLogTest(unittest.TestCase):
    def test_set_weights_records_audit(self):
        ...
        entry = scorer.set_weights(w_after, actor_id="leader1",
                                   reason="产能优先，提升产量与换岗权重")
...
class ExplanationTest(unittest.TestCase):
    def test_explain_passed_candidate_references_real_values(self):
        cand = Candidate(person_id="p1", device_id="d1", task_id="t1",
                        station_id="s1", passed=True)
        cand.score = 0.42
        cand.score_breakdown = {
            "production_score": 0.5, "on_time_score": 0.8,
            "safety_risk": 0.1, "body_load": 0.71, "travel_distance": 12.3,
            "changeover_cost": 0.0,
            "w1_production_contrib": 0.5, "w2_on_time_contrib": 0.8,
            "w3_safety_contrib": -0.1, "w4_body_load_contrib": -0.71,
            "w5_travel_contrib": -0.615, "w6_changeover_contrib": 0.0,
            "total": 0.42, "body_load_baseline": 0.55,
        }
        ...
        self.assertTrue(any("\u4e00" <= ch <= "\u9fff"
                            for r in exp.reasons for ch in r))

    def test_explain_failed_candidate_includes_violation(self):
        v = ConstraintViolation(SKILL, "p2", "人员 p2 缺少任务所需技能：lifting")
        cand = Candidate(person_id="p2", device_id="d1", task_id="t1",
                        station_id="s1", violations=[v], passed=False)
        exp = explain_candidate(cand)
        text = "".join(exp.reasons)
    ...
    def test_explain_plan_summarizes(self):
        c1 = Candidate(person_id="p1", device_id="d1", task_id="t1",
                      station_id="s1", passed=True, score=0.9,
                      score_breakdown={"total": 0.9, "body_load": 0.3,
                                       "body_load_baseline": 0.5})
        c2 = Candidate(person_id="p2", device_id="d1", task_id="t1",
                      station_id="s1", passed=False,
...
class SchedulerTest(unittest.TestCase):
    ...
    def _propose(self, sched):
        task = {"task_id": "t1", "required_skills": {"lifting"},
                "station_id": "s1", "zone_id": "z1"}
        persons = [{"person_id": "p1"}, {"person_id": "p2"}]
        devices = [{"device_id": "d1", "model": "exoA"}]
        ctx = {
            "persons_state": {
    ...
    def test_feedback_recorded(self):
        ...
        fb = sched.feedback(req.request_id,
                            {"actual_production": 120, "on_time": True})
