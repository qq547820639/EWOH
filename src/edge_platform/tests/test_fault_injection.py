...
class TimestampBackwardsTest(unittest.TestCase):
    ...
    def test_generates_valid_frame_with_earlier_ts(self):
        inj = WireInjector(device_id="EXO-TB-01", source_label="controlled_test",
                           start_ts_ms=1_000_000, hz=20.0)
        # 先发一帧正常遥测建立基线
        first = inj.telemetry()
    ...
    def test_adapter_marks_timestamp_backward(self):
        inj = WireInjector(device_id="EXO-TB-02", source_label="controlled_test",
                           start_ts_ms=1_000_000, hz=20.0)
...
class NanFieldTest(unittest.TestCase):
    ...
    def test_nan_field_multiple_fields(self):
        ...
        frame = adapter.to_unified(
            {"pitch_deg": float("nan"), "torque_nm": float("inf"),
             "battery_pct": 50, "roll_deg": 5.0},
            ts_ms=1_000_000)
        self.assertEqual(frame.quality["status"], "invalid")
...
class FirmwareUpgradeTest(unittest.TestCase):
    ...
    def test_generates_valid_ident_frame_with_new_version(self):
        inj = WireInjector(device_id="EXO-FW-01", start_ts_ms=1_000_000,
    ...
    def test_adapter_records_firmware_upgraded_event(self):
        adapter = NyExoA1Adapter("EXO-FW-02", source_type="controlled_test",
                                 firmware_version="1.0.0")
        inj = WireInjector(device_id="EXO-FW-02", source_label="controlled_test",
                           start_ts_ms=1_000_000, firmware_version="1.0.0")
    ...
    def test_rewrite_existing_ident_bytes(self):
        inj = WireInjector(device_id="EXO-FW-03", start_ts_ms=1_000_000,
                           firmware_version="1.0.0")
        original = inj.ident()
    ...
    def test_invalid_frame_bytes_raises(self):
        inj = WireInjector(device_id="EXO-FW-04", start_ts_ms=1_000_000,
                           firmware_version="1.0.0")
        # 用一个 TELEMETRY 帧拒绝
