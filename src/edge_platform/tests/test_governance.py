...
class ConsentManagerTest(unittest.TestCase):
    def test_grant_allows_and_revoke_denies(self):
        mgr = ConsentManager()
        rec = mgr.grant(
            "p1", [ConsentPurpose.TELEMETRY], ["load.torque"], "leader1",
    ...
    def test_revoke_produces_revocation_job_with_delete_and_anonymize(self):
        ...
        # 用途级动作覆盖三个用途
        purpose_actions = {a["data_class"]: a["action"] for a in job.actions
                           if a["data_class"] != "field"}
...
class RetentionManagerTest(unittest.TestCase):
    ...
    def test_high_freq_telemetry_purge_due(self):
        ...
        records = [
            {"data_class": DataClass.HIGH_FREQ_TELEMETRY,
             "record_ts": _ts_days_ago(40), "record_id": "t1"},   # 过期
            {"data_class": DataClass.HIGH_FREQ_TELEMETRY,
             "record_ts": _ts_days_ago(10), "record_id": "t2"},   # 未过期
        ]
        due_ids = {r["record_id"] for r in mgr.purge_due(records)}
    ...
    def test_version_history_retained(self):
        mgr = RetentionManager()
        p1 = RetentionPolicy(data_class=DataClass.HIGH_FREQ_TELEMETRY,
                             retention_days=7, policy_id="p1")
        p2 = RetentionPolicy(data_class=DataClass.HIGH_FREQ_TELEMETRY,
                             retention_days=14, policy_id="p2")
        mgr.register(p1)
        mgr.register(p2)
        # 历史保留两版
        ...
        ts = "2026-01-01T00:00:00.000+00:00"
        self.assertEqual(mgr.expire_by(DataClass.HIGH_FREQ_TELEMETRY, ts) - ts_to_ms(ts),
                         14 * 86400 * 1000)
...
class ModelRegistryTest(unittest.TestCase):
    def test_register_candidate(self):
        reg = ModelRegistry()
        rec = ModelRecord(
            model_type=ACTION_CLASSIFIER, version="1.0.0",
