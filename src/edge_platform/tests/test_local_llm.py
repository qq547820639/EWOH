...
def _make_rule_registry():
    reg = RuleRegistry()
    reg.register(PostureThresholdRule(config={"trunk_pitch_deg": 45.0,
                                              "sustained_sec": 5}))
...
class SummarizeEventsTest(unittest.TestCase):
    def test_summarize_events_content_correct(self):
        ...
        ast = _make_assistant()
        events = [
            {"event_id": "E1", "severity": "L1", "type": "posture",
             "ts": "2026-07-31T01:00:00.000+00:00", "message": "前倾"},
            {"event_id": "E2", "severity": "L2", "type": "load",
             "ts": "2026-07-31T02:00:00.000+00:00", "message": "高负荷"},
            {"event_id": "E3", "severity": "L1", "type": "posture",
             "ts": "2026-07-31T03:00:00.000+00:00", "message": "前倾"},
        ]
...
class ExplainScheduleTest(unittest.TestCase):
    ...
    def test_explain_schedule_references_schedule_request(self):
        ...
        req = _make_schedule_request()
        resp = ast.explain_schedule(req)
        sched_refs = [r for r in resp.source_refs
                      if r["kind"] == "schedule_request"]
    ...
    def test_explain_schedule_uses_candidate_explanation(self):
        ...
        resp = ast.explain_schedule(req)
        # 通过候选的评分应出现在解释内容中
        self.assertTrue(any(str(c.candidate_id) in resp.content
                            for c in passed))
...
class HistoricalCasesTest(unittest.TestCase):
    def test_find_historical_cases_similarity_sort(self):
        ...
        ast = _make_assistant()
        cases = [
            {"case_id": "C1", "title": "外骨骼高负荷事件",
             "summary": "人员高负荷 搬运 持续超限"},
            {"case_id": "C2", "title": "无关案例",
             "summary": "网络波动 设备离线"},
            {"case_id": "C3", "title": "高负荷搬运",
             "summary": "人员高负荷 搬运"},
        ]
        ...
        # 所有案例均被引用
        case_refs = [r for r in resp.source_refs
                     if r["kind"] == "historical_case"]
...
class GenerateReportTest(unittest.TestCase):
    def test_generate_report_structured_output(self):
        ...
        data = {
            ...
            "events": [{"event_id": "E1", "severity": "L1"},
                       {"event_id": "E2", "severity": "L2"}],
            "schedule_requests": [
...
class AuditLogTest(unittest.TestCase):
    def test_audit_log_records_all_calls(self):
        ...
        self.assertEqual(intents, [LLMIntent.NATURAL_QUERY.value,
                                   LLMIntent.EVENT_SUMMARY.value])
...
class SafetyInvariantTest(unittest.TestCase):
    def test_all_outputs_carry_safety_flags(self):
        ...
        responses = [
            ...
            ast.find_historical_cases(
                "高负荷", [{"case_id": "C1", "title": "高负荷"}]),
        ...
        for resp in responses:
            self.assertTrue(resp.generated_by_llm,
                            "intent=%s 缺少 generated_by_llm" % resp.intent)
            self.assertTrue(resp.not_for_direct_control,
                            "intent=%s 缺少 not_for_direct_control" % resp.intent)
            # 所有响应都附免责声明
