...
from edge_platform.spatial import Pose, BoundingBox
...
def mk_ctx(fused_state=None, device_state=None, task_state=None,
           spatial_registry=None, zone_registry=None, now_ts=None):
    """构造评估上下文。"""
    return {
...
class PostureThresholdTest(unittest.TestCase):
    def test_sustained_high_pitch_triggers(self):
        ...
        rule = PostureThresholdRule(config={"trunk_pitch_deg": 45.0,
                                            "sustained_sec": 5})
        findings = []
        for i in range(7):
            ctx = mk_ctx(
                fused_state={
                    "person_id": "P1", "device_id": "D1", "station_id": "STN-1",
        ...
        # 已触发未收口 → 后续帧不重复触发
        again = rule.evaluate(mk_ctx(
            fused_state={"person_id": "P1", "trunk_pitch_deg": 60.0,
                         "pose": Pose()},
    ...
    def test_brief_spike_does_not_trigger(self):
        ...
        # 恢复正常姿态 → 状态机复位
        ctx = mk_ctx(
            fused_state={"person_id": "P1", "trunk_pitch_deg": 10.0,
                         "pose": Pose()},
    ...
    def test_missing_pitch_returns_none(self):
        ...
        ctx = mk_ctx(fused_state={"person_id": "P1"},
...
class BatteryPredictionTest(unittest.TestCase):
    def test_high_drain_low_battery_predicts_soon(self):
        ...
        rule = BatteryPredictionRule(config={"low_threshold": 20.0,
                                             "horizon_min": 15})
        ctx = mk_ctx(
            fused_state={
                "person_id": "P1", "device_id": "D1", "station_id": "STN-1",
    ...
    def test_internal_drain_estimate_from_history(self):
        ...
        rule.evaluate(mk_ctx(
            fused_state={"person_id": "P1", "device_id": "D1",
                         "battery_percent": 50.0},
            now_ts=ms_to_ts(BASE_TS),
        ))
        finding = rule.evaluate(mk_ctx(
            fused_state={"person_id": "P1", "device_id": "D1",
...
class ZoneViolationTest(unittest.TestCase):
    ...
    def test_person_inside_forbidden_zone_fires(self):
        ...
        ctx = mk_ctx(
            fused_state={"person_id": "P1", "device_id": "D1",
                         "pose": Pose(x=2.0, y=2.0, confidence=0.95)},
    ...
    def test_dict_pose_supported(self):
        ...
        ctx = mk_ctx(
            fused_state={"person_id": "P1", "device_id": "D1",
                         "pose": {"x": 1.0, "y": 1.0, "confidence": 0.8}},
            zone_registry=self._zones(),
            now_ts=ms_to_ts(BASE_TS),
        )
        finding = rule.evaluate(ctx)
    ...
    def test_non_forbidden_zone_ignored(self):
        rule = ZoneViolationRule()
        zones = [{"zone_id": "Z-NORMAL", "bbox": BoundingBox(0, 0, 5, 5),
                  "status": "active"}]
        ctx = mk_ctx(
            fused_state={"person_id": "P1",
                         "pose": Pose(x=2.0, y=2.0)},
            zone_registry=zones,
            now_ts=ms_to_ts(BASE_TS),
        )
        self.assertIsNone(rule.evaluate(ctx))
    ...
    def test_leave_then_reenter(self):
        ...
        # 仍在禁区 → 不重复触发
        f2 = rule.evaluate(mk_ctx(
            fused_state={"person_id": "P1",
                         "pose": Pose(x=3.0, y=3.0)},
            zone_registry=self._zones(),
...
class CumulativeLoadIntegralTest(unittest.TestCase):
    ...
    def test_below_threshold_no_finding(self):
        ...
        for i in range(5):
            ctx = mk_ctx(
                fused_state={"person_id": "P1", "load_score": 1.0,
                             "shift_start_ts": ms_to_ts(BASE_TS)},
                now_ts=ms_to_ts(BASE_TS + i * 1000),
            )
            self.assertIsNone(rule.evaluate(ctx))
    ...
    def test_shift_change_resets_accumulator(self):
        ...
        findings = []
        for i in range(5):
            f = rule.evaluate(mk_ctx(
                fused_state={"person_id": "P1", "load_score": 1.0,
                             "shift_start_ts": ms_to_ts(BASE_TS + 100000)},
...
class RuleRegistryTest(unittest.TestCase):
    def test_register_two_rules_and_evaluate_all(self):
        reg = RuleRegistry()
        r1 = PostureThresholdRule(config={"trunk_pitch_deg": 45.0,
                                          "sustained_sec": 1})
    ...
    def test_version_history_retained(self):
        reg = RuleRegistry()
        v1 = PostureThresholdRule(config={"sustained_sec": 5},
                                  rule_version="spatial-rule-v1.0")
        v2 = PostureThresholdRule(config={"sustained_sec": 3},
                                  rule_version="spatial-rule-v1.1")
        reg.register(v1)
        reg.register(v2)
        # 版本历史保留（不覆盖）
        self.assertEqual(reg.versions("POSTURE_THRESHOLD"),
                         ["spatial-rule-v1.0", "spatial-rule-v1.1"])
    ...
    def test_enable_disable_versioned(self):
        ...
        self.assertFalse(reg.is_enabled("POSTURE_THRESHOLD",
                                        "spatial-rule-v1.0"))
        self.assertTrue(reg.is_enabled("POSTURE_THRESHOLD",
                                       "spatial-rule-v1.1"))
        enabled = reg.enabled()
    ...
    def test_evaluate_all_respects_disabled(self):
        ...
        zones = [{"zone_id": "Z1",
                  "bbox": BoundingBox(0, 0, 5, 5), "status": "forbidden"}]
        ctx = mk_ctx(
            fused_state={"person_id": "P1",
                         "pose": Pose(x=2.0, y=2.0)},
            zone_registry=zones,
...
class AllRulesSmokeTest(unittest.TestCase):
    ...
    def test_high_load_duration(self):
        ...
        for i in range(3):
            f = rule.evaluate(mk_ctx(
                fused_state={"person_id": "P1", "device_id": "D1",
                             "assist_level": 0.9, "torque_nm": 30.0},
                now_ts=ms_to_ts(BASE_TS + i * 1000),
            ))
            if f is not None:
                findings.append(f)
    ...
    def test_offline_detection(self):
        ...
        ctx = mk_ctx(
            fused_state={"person_id": "P1", "device_id": "D1"},
            device_state={"device_id": "D1",
                          "last_seen_ts": ms_to_ts(BASE_TS - 60000)},
            now_ts=ms_to_ts(BASE_TS),
        )
        finding = rule.evaluate(ctx)
        self.assertIsNotNone(finding)
    ...
    def test_station_dwell(self):
        rule = StationDwellRule(config={"dwell_slack_sec": 0,
                                        "default_expected_dwell_sec": 60})
        ctx = mk_ctx(
            fused_state={"person_id": "P1", "device_id": "D1",
                         "station_id": "STN-1",
    ...
    def test_task_timeout_complete_overdue(self):
        ...
        ctx = mk_ctx(
            ...
            task_state={"task_id": "TASK-2", "person_id": "P1",
    ...
    def test_rulefinding_to_dict(self):
        f = RuleFinding(rule_id="X", rule_version="v1", severity="L1",
                        person_id="P1", device_id="D1", station_id="S1",
                        message="m", evidence={"a": 1},
