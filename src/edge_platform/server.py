    ...
    class Handler(SimpleHTTPRequestHandler):
        ...
        def _flush_post_audit(self):
            ...
            if getattr(self, "_post_audit_pending", False):
                ...
                self._audit("POST " + urlparse(self.path).path,
        ...
        def _new_error(self, code, message, status):
            ...
            return self.send_json({"error": {"code": code, "message": message,
                                             "request_id": getattr(self, "_request_id", "")}}, status)

        # ---- Task 16.6 操作人身份与审计 ----
        def _actor(self):
            return get_actor_from_request(self)

        def _audit(self, action, target_type=None, target_id=None,
                   before=None, after=None, result="success"):
            """记录审计日志（失败不影响主流程）。"""
            try:
                ctx.storage.insert_audit_log(
                    action=action, actor_id=self._actor(),
        ...
        def do_POST(self):
            ...
            try:
                ...
                if p == "/api/tasks/confirm":
                    ...
                    return
                if p == "/api/query":
                    self.send_json(services.answer(ctx.storage, payload.get("question", ""),
                ...
                self._post_audit_pending = False
                self._audit("POST " + p, target_type=getattr(self, "_audit_target_type", "api"),
                            target_id=getattr(self, "_audit_target_id", None), result="error")
                return self.send_json({"error": "请求处理失败", "detail": str(e)}, 500)
        ...
        def _refresh_device_stats(self):
            ...
            ctx.metrics.set_device_stats(online, offline,
        ...
        def api_inference_metrics(self):
            ...
            try:
                for d in ctx.storage.list_devices():
                    inf_rows.extend(ctx.storage.query_inference(
                        d.get("device_id"), services.iso(start), services.iso(end), limit))
            except Exception:
                inf_rows = []
            total = len(inf_rows)
        ...
        def api_auth_login(self, payload):
            ...
            if sm is not None:
                ...
                session = sm.verify(token)
                user = {"user_id": session.user_id, "username": username,
                        "role": session.role}
        ...
        def api_auth_refresh(self):
            ...
            if sm is not None:
                ...
                new_token = sm.create(User(user_id=session.user_id, username=session.user_id,
                                           role=session.role, display_name=""))
        ...
        def api_export_post(self, payload):
            ...
            slice_ = ctx.storage.export_slice(device_id, services.iso(parse_ts(start)),
                                              services.iso(parse_ts(end)))
            if isinstance(slice_, list):
                slice_ = {"records": slice_}
            ...
            out = {"export_type": "raw_slice", "device_id": device_id, "start": start, "end": end,
                   "format": fmt, "exported_at": now_iso(), "slice": slice_,
        ...
        def api_event_detail_v2(self, event_id):
            ...
            if t0 and dev_id:
                ...
                records = [services.norm_telemetry(r) for r in rows]
            evt["source_label"] = SOURCE_LABELS.get(evt.get("source_type"), evt.get("source_type"))
            handlings = (ctx.storage.list_event_handlings(event_id)
                         if hasattr(ctx.storage, "list_event_handlings") else [])
            return self.send_json({"event": evt, "evidence_window_sec": EVIDENCE_WINDOW_SEC,
                                   "evidence_records": records, "handlings": handlings, "now": now_iso()})
        ...
        def api_event_status_v2(self, event_id, payload):
            ...
            handling = {"handled_by": handler_id, "handled_at": now_iso(),
                        "action": action, "comment": comment}
            ctx.storage.update_event_status(event_id, status, handling)
            if hasattr(ctx.storage, "insert_event_handling"):
                ctx.storage.insert_event_handling(event_id, handler_id, action,
                                                  comment=comment, audit_ref=getattr(self, "_request_id", None))
        ...
        def api_event_comment(self, event_id, payload):
            ...
            rec = None
            if hasattr(ctx.storage, "insert_event_handling"):
                rec = ctx.storage.insert_event_handling(event_id, author_id, "comment",
                                                        comment=comment, audit_ref=getattr(self, "_request_id", None))
            self._audit_target_type = "risk_event"
            self._audit_target_id = event_id
        ...
        def api_audit(self):
            ...
            items = ctx.storage.list_audit_logs(action=action, actor_id=actor_id,
                                                limit=limit, offset=offset) \
                if hasattr(ctx.storage, "list_audit_logs") else []
        ...
        def api_models(self):
            ...
            return self.send_json({"items": items[offset:offset + limit], "limit": limit,
                                   "offset": offset, "now": now_iso()})
...
def build_server(addr, ctx, tls_cert=None, tls_key=None):
    ...
    if tls_cert and tls_key:
        ...
        httpd.socket = ssl.wrap_socket(httpd.socket, certfile=tls_cert,
                                       keyfile=tls_key, server_side=True)
    return httpd
