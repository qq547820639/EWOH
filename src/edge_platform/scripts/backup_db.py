...
def _parse_args(argv):
    p = argparse.ArgumentParser(
        description="EWOH SQLite 数据库备份/恢复/校验/列表工具（Task 34）")
    p.add_argument("--action", required=True,
                   choices=["backup", "restore", "verify", "list"],
                   help="操作类型：backup / restore / verify / list")
    p.add_argument("--db", default=None,
                   help="数据库路径（backup/restore/verify 需要）")
    p.add_argument("--output", default="backups",
                   help="备份输出目录（backup）或备份列表目录（list）；默认 backups/")
    p.add_argument("--backup", default=None,
                   help="恢复时使用的备份 .db 文件路径（restore 需要）")
...
def main(argv=None):
    ...
    if args.action == "backup":
        ...
        print("[ok] JSON 副本: %s" % (os.path.splitext(path)[0] + ".json"))
        ok, msg = mgr.verify(path)
        print("[verify] {} -> {}".format("PASS" if ok else "FAIL", msg))
    ...
    if args.action == "verify":
        ...
        print("[{}] {} -> {}".format("PASS" if ok else "FAIL", args.db, msg))
        return 0 if ok else 4
