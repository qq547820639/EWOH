...
class EnvSensorAdapter(BaseAdapter):
    ...
    def __init__(self, asset, source_type=None):
        if isinstance(asset, str):
            asset = EnvSensorAsset(sensor_id=asset, station_id="",
                                   source_type=source_type or "real")
        else:
            if source_type is not None:
                asset.source_type = source_type
        super().__init__(asset.sensor_id, source_type=asset.source_type,
                         model="ENV-GENERIC")
        self.asset = asset
    ...
    def _enqueue_raw(self, raw):
        ...
        try:
            msg = parse_env_reading(raw, default_sensor_id=self.asset.sensor_id,
...
class SimulatedEnvSensorAdapter(EnvSensorAdapter):
    ...
    def __init__(self, sensor_id, station_id, hz=1.0, source_type="simulated"):
        asset = EnvSensorAsset(sensor_id=sensor_id, station_id=station_id,
