...
def _load_labels(db_path):
    ...
    try:
        rows = c.execute(
            ...
            " JOIN collection_session s ON l.session_id = s.session_id").fetchall()
        sess = c.execute(
            "SELECT session_id, start_ts, end_ts FROM collection_session").fetchall()
    finally:
        c.close()
...
def _assign_splits(persons, split):
    ...
    n = len(persons)
    if n < 3:
        raise ValueError("人员数 %d 不足 3，无法做人员独立 train/val/test 划分" % n)
    ordered = sorted(persons,
                     key=lambda p: int(hashlib.sha256(p.encode("utf-8")).hexdigest(), 16))
...
def export_dataset(storage, out_dir, version, window_sec=2, step_sec=1,
                   split=(0.6, 0.2, 0.2)):
    ...
    for lb in labels:
        ...
        msgs = storage.query_telemetry(lb["device_id"], lb["start_ts"], lb["end_ts"],
                                       QUERY_LIMIT)
        ...
        for i in range(0, len(msgs) - win_n + 1, step_n):
            ...
            if feats is None:
                win_dropped += 1
                continue
            samples.append({"features": feats, "label": lb["action_code"],
                            "person_id": lb["person_id"], "session_id": lb["session_id"]})
    if not samples:
    ...
    ds_dir = os.path.join(out_dir, "dataset-v%s" % version)
    ...
    manifest = {
        ...
        "quality": {
            ...
            "windows_total": win_total, "windows_dropped": win_dropped,
        },
    }
