...
_METRIC_DEFS = [
    # 系统级
    ("ewoh_uptime_seconds", "gauge",
     "Platform uptime since collector start in seconds.",
     "uptime_seconds", None),
    ("ewoh_db_count", "gauge",
     "Database row counts grouped by table.",
     "db_counts", ("table",)),
    # 设备级
    ("ewoh_device_online_count", "gauge",
     "Number of currently online devices.",
     "online_count", None),
    ("ewoh_device_offline_count", "gauge",
     "Number of currently offline devices.",
     "offline_count", None),
    ("ewoh_device_avg_packet_loss_pct", "gauge",
     "Average packet loss percentage across devices.",
     "avg_packet_loss_pct", None),
    ("ewoh_device_low_battery_count", "gauge",
     "Number of devices with low battery.",
     "low_battery_count", None),
    # 推理级
    ("ewoh_inference_count_total", "counter",
     "Total number of inferences recorded since collector start.",
     "inference_count", None),
    ("ewoh_inference_p50_ms", "gauge",
     "P50 inference latency in milliseconds.",
     "inference_p50_ms", None),
    ("ewoh_inference_p95_ms", "gauge",
     "P95 inference latency in milliseconds.",
     "inference_p95_ms", None),
    ("ewoh_inference_unknown_count_total", "counter",
     "Total number of inferences labeled unknown.",
     "unknown_count", None),
    ("ewoh_inference_error_count_total", "counter",
     "Total number of inferences that raised an error.",
     "error_count", None),
    # 业务级
    ("ewoh_event_open_count", "gauge",
     "Number of currently open risk events.",
     "open_event_count", None),
    ("ewoh_event_open_total", "counter",
     "Cumulative number of events opened since collector start.",
     "event_open_total", None),
    ("ewoh_event_avg_close_hours", "gauge",
     "Average hours to close an event.",
     "avg_event_close_hours", None),
    ("ewoh_assignment_recommendation_count_total", "counter",
     "Total number of task recommendations generated.",
     "recommendation_count", None),
    ("ewoh_assignment_confirmed_count_total", "counter",
     "Total number of task assignments confirmed by humans.",
     "confirmed_count", None),
    ("ewoh_assignment_adoption_rate", "gauge",
     "Fraction of recommendations confirmed (confirmed/recommended).",
     "assignment_adoption_rate", None),
]
            ...
            if labels_spec is None:
                ...
                if not value:
                    ...
                    for label_value, v in sorted(value.items()):
                        ...
                        lines.append(
                            f'{prom_name}{{{label_name}="{_format_label_value(label_value)}"}} '
                            f'{_format_value(v)}')
            else:
                # 期望 dict 但拿到标量：回落为单值
                lines.append(f"{prom_name} {_format_value(value)}")
