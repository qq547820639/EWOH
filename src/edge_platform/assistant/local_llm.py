...
class LocalLLMAssistant:
    ...
    def _make_response(self, intent_value, content, source_refs,
                       caveats=None, confidence=1.0):
        """统一构造 LLMResponse 并记审计；强制安全不变量。"""
        intent = (intent_value.value
                  if isinstance(intent_value, LLMIntent) else str(intent_value))
    ...
    def query(self, question, context):
        ...
        if not context or not str(question).strip():
            return self._insufficient_context(
                LLMIntent.NATURAL_QUERY, "自然语言查询缺少问题或上下文")
        ...
        for pid, pstate in persons_state.items():
            pstate = pstate or {}
            for fld in ("current_load", "expected_production_uplift",
                        "on_time_probability", "safety_risk", "battery_percent"):
                if fld in pstate and pstate[fld] is not None:
                    source_refs.append(_ref("person_state", pid, fld))
                    data_lines.append("  - 人员 %s 的 %s = %s" % (
                        pid, fld, _fmt_num(pstate[fld])))
            if "pose" in pstate and pstate["pose"] is not None:
                source_refs.append(_ref("person_state", pid, "pose"))
                data_lines.append("  - 人员 %s 位置已记录" % pid)
        ...
        for mk, mv in metrics.items():
            ...
            data_lines.append("  - 指标 %s = %s" % (mk, _fmt_num(mv)))

        events = _get(context, "events", []) or []
        for ev in events:
            eid = _get(ev, "event_id", _get(ev, "id", "?"))
            source_refs.append(_ref("event", eid, "summary"))
            data_lines.append("  - 事件 %s：%s" % (
                eid, _get(ev, "message", _get(ev, "type", ""))))

        if not source_refs:
            return self._insufficient_context(
                LLMIntent.NATURAL_QUERY, "上下文中无可引用的真实数据点")
        ...
        return self._make_response(
            LLMIntent.NATURAL_QUERY, content, source_refs, confidence=0.9)
    ...
    def summarize_events(self, events, time_window=None):
        ...
        if not events:
            return self._insufficient_context(
                LLMIntent.EVENT_SUMMARY, "事件总结缺少事件列表")
        ...
        sev_summary = "、".join("%s×%d" % (k, v)
                                for k, v in sorted(by_severity.items()))
        type_summary = "、".join("%s×%d" % (k, v)
                                 for k, v in sorted(by_type.items()))
        time_range = ""
        if ts_list:
            time_range = "（时间范围 %s ~ %s）" % (ts_list[0], ts_list[-1])
        window_note = ""
        if time_window is not None:
            window_note = "；统计窗口：%s" % str(time_window)

        prompt = (
            "事件总结%s：\n"
            "共 %d 条事件；按严重级别：%s；按类型：%s%s\n"
            "所有事件均来自传入事件列表，未虚构。" % (
                time_range, len(events), sev_summary, type_summary, window_note)
        )
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.EVENT_SUMMARY, content, source_refs, confidence=0.95)
    ...
    def explain_schedule(self, schedule_request):
        ...
        if schedule_request is None:
            return self._insufficient_context(
                LLMIntent.SCHEDULE_EXPLANATION, "调度解释缺少 ScheduleRequest")
        ...
        if recommended is not None:
            ...
            rec_lines.append(
                "  推荐方案：候选 %s（人员 %s + 设备 %s），综合评分 %s" % (
                    cid, pid, did, _fmt_num(score)))
            source_refs.append(_ref("schedule_request", rid, "ranked_candidate_ids"))
        else:
            rec_lines.append("  无通过硬约束的候选，无可推荐方案")

        prompt = (
            "调度方案解释（ScheduleRequest %s）：\n"
            "触发原因：\n%s\n"
            "任务：%s\n"
            "候选评分排序（共 %d 个，引用 Scheduler 已生成结果，未重新评分/未生成新候选）：\n%s\n"
            "推荐方案：\n%s\n"
        ...
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.SCHEDULE_EXPLANATION, content, source_refs,
    ...
    def retrieve_rules(self, query, rule_registry):
        ...
        if rule_registry is None:
            return self._insufficient_context(
                LLMIntent.RULE_RETRIEVAL, "规则检索缺少规则注册表")
        if not str(query).strip():
            return self._insufficient_context(
                LLMIntent.RULE_RETRIEVAL, "规则检索缺少查询关键词")
        ...
        if not all_rules:
            return self._insufficient_context(
                LLMIntent.RULE_RETRIEVAL, "规则注册表为空")
        ...
        for rule in all_rules:
            ...
            for ck, cv in config.items():
                haystack_parts.append("%s:%s" % (ck, cv))
        ...
        if not matched:
            prompt = "规则检索：查询 '%s' 未匹配到任何规则（注册表共 %d 条）。" % (
                query, len(all_rules))
            content = self._backend(prompt)
            return self._make_response(
                LLMIntent.RULE_RETRIEVAL, content, [],
                caveats=["未匹配到规则；可尝试调整检索关键词。"],
                confidence=0.5)
        ...
        for rule in matched:
            ...
            sev = _get(rule, "severity", "")
            config = _get(rule, "config", {}) or {}
            cfg_str = ", ".join("%s=%s" % (k, v) for k, v in config.items()) or "(默认)"
            match_lines.append("  - %s v%s [%s] 配置: %s" % (rid, rver, sev, cfg_str))

        prompt = (
            "规则检索：查询 '%s'，注册表共 %d 条，匹配 %d 条：\n%s\n"
            "所有规则定义来自规则注册表，未虚构。" % (
                query, len(all_rules), len(matched), "\n".join(match_lines))
        )
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.RULE_RETRIEVAL, content, source_refs, confidence=0.9)
    ...
    def find_historical_cases(self, current_scenario, case_database):
        ...
        if not case_database:
            return self._insufficient_context(
                LLMIntent.HISTORICAL_CASE, "历史案例库为空")
        if not current_scenario:
            return self._insufficient_context(
                LLMIntent.HISTORICAL_CASE, "当前场景描述为空")
        ...
        for sim, cid, case, _ in scored:
            ...
            case_lines.append("  - [相似度 %.2f] %s（%s）" % (sim, cid, title))

        prompt = (
            "历史案例检索：当前场景特征词 %d 个，案例库 %d 条，按相似度降序：\n%s\n"
            "所有案例来自案例库，相似度基于关键词重叠计算，未虚构。" % (
                len(cur_words), len(case_database), "\n".join(case_lines))
        )
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.HISTORICAL_CASE, content, source_refs, confidence=0.85)
    ...
    def generate_shift_handover(self, shift_data):
        ...
        if not shift_data:
            return self._insufficient_context(
                LLMIntent.SHIFT_HANDOVER, "交接班数据为空")
        ...
        if events:
            lines.append("本班次关键事件（共 %d 条）：" % len(events))
        ...
        if todos:
            lines.append("待办事项（共 %d 项）：" % len(todos))
            for i, todo in enumerate(todos):
                tid = _get(todo, "todo_id", _get(todo, "id", "todo-%d" % (i + 1)))
                source_refs.append(_ref("todo", tid, "description"))
                lines.append("  - %s" % _get(todo, "description", str(todo)))
        else:
            lines.append("待办事项：（无）")
        ...
        if risks:
            lines.append("风险提示（共 %d 项）：" % len(risks))
            for i, risk in enumerate(risks):
                rid = _get(risk, "risk_id", _get(risk, "id", "risk-%d" % (i + 1)))
                source_refs.append(_ref("risk", rid, "description"))
                lines.append("  - %s" % _get(risk, "description", str(risk)))
        ...
        prompt = (
            "交接班摘要（班次 %s）：\n%s\n"
            "所有内容来自交接班数据，未虚构。" % (shift_id, "\n".join(lines))
        )
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.SHIFT_HANDOVER, content, source_refs, confidence=0.9)
    ...
    def hypothesize_root_cause(self, anomaly, context):
        ...
        if not anomaly:
            return self._insufficient_context(
                LLMIntent.ROOT_CAUSE_HYPOTHESIS, "异常描述为空")
        ...
        if device_faults:
            ev_text = "、".join("%s=%s" % (d, s) for d, s in device_faults)
            hypotheses.append((
                0.8, "设备异常：检测到 %s 状态异常（%s）" % (atype, ev_text),
                device_faults))
        ...
        if high_load_persons:
            ev_text = "、".join("%s=%.2f" % (pid, load_val)
                                for pid, load_val in high_load_persons)
            hypotheses.append((
                0.6, "人员高负荷：人员 %s 累计负荷偏高（%s），可能引发 %s" % (
                    ev_text, ev_text, atype),
                high_load_persons))
        ...
        if len(same_type_events) >= 2:
            hypotheses.append((
                0.5, "事件聚集：近期同类事件 %s 多次发生（%s），疑为系统性根因" % (
                    atype, ", ".join(same_type_events)),
                same_type_events))
        ...
        if out_of_range:
            ev_text = "、".join("%s=%s" % (t, f) for t, f in out_of_range)
            hypotheses.append((
                0.7, "传感器异常：遥测项 %s 异常（%s）" % (ev_text, ev_text),
                out_of_range))

        # 无证据时给出默认低置信假设
        if not hypotheses:
            hypotheses.append((
                0.2, "证据不足：基于异常类型 %s 暂无明确线索，需人工现场核实" % atype,
                []))
        ...
        for i, (prob, text, _) in enumerate(hypotheses):
            hypo_lines.append(
                "  假设 %d（可能性 %.2f）：%s" % (i + 1, prob, text))
        hypo_lines.append(
            "  ⚠ 以上均为假设，需人工核实；大模型不得据此直接控制设备或调整调度。")

        prompt = (
            "异常根因假设（异常 %s，类型 %s）：\n%s\n"
            "所有证据来自传入上下文，未虚构传感器数据。" % (
                aid, atype, "\n".join(hypo_lines))
        )
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.ROOT_CAUSE_HYPOTHESIS, content, source_refs,
            caveats=[
    ...
    def generate_report(self, report_type, data, period=None):
        ...
        if not data:
            return self._insufficient_context(
                LLMIntent.REPORT_GENERATION, "报告数据为空")
        ...
        lines.append("报告类型：%s | 周期：%s" % (
            {"daily": "日报", "weekly": "周报", "event": "事件报告"}[rtype],
            period_str))
        ...
        if metrics:
            ...
            for mk, mv in metrics.items():
                ...
                lines.append("  - %s: %s" % (mk, _fmt_num(mv)))
        else:
        ...
        if events:
            lines.append("事件统计（共 %d 条）：" % len(events))
            by_sev = {}
            for ev in events:
                sev = _get(ev, "severity", "UNKNOWN")
        ...
        if schedules:
            ...
            for req in schedules:
                ...
                if status == "EXECUTED":
                    executed += 1
            lines.append("  - 已确认 %d / 已执行 %d / 总计 %d" % (
                confirmed, executed, len(schedules)))

        # 总结建议（辅助性，不直接控制）
        summary = _get(data, "summary", "")
        if summary:
            source_refs.append(_ref("report", rtype, "summary"))
            lines.append("总结：%s" % summary)

        prompt = (
            "%s（%s）：\n%s\n"
            "所有数据来自传入报告数据，未虚构；本报告仅供辅助决策，不得直接用于控制。" % (
                {"daily": "生产日报", "weekly": "生产周报",
                 "event": "事件报告"}[rtype],
                period_str, "\n".join(lines))
        )
        content = self._backend(prompt)
        return self._make_response(
            LLMIntent.REPORT_GENERATION, content, source_refs, confidence=0.9)
