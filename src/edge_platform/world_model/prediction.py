...
class Predictor:
    ...
    def _make(self, target_entity_id, prediction_type, horizon_min,
              predicted_value, probability, confidence, assumptions):
    ...
    def predict_low_battery(self, device_id, battery_pct, drain_per_min,
        ...
        if drain > 0:
            ...
            probability = min(0.99, 0.5 + (horizon_min - minutes_to_threshold)
                              / horizon_min * 0.49)
            ...
            confidence = 0.4
        return self._make(
            device_id, "LOW_BATTERY", horizon_min,
            predicted_value={
                "predicted_battery_pct": round(predicted_battery, 2),
                "threshold": threshold,
                "minutes_to_threshold": round(minutes_to_threshold, 2)
                    if minutes_to_threshold is not None else None,
            },
            probability=probability,
            confidence=confidence,
            assumptions={
    ...
    def predict_task_delay(self, task_id, progress_pct, elapsed_min, sla_min):
        ...
        if progress_pct <= 0:
            ...
            if elapsed_min >= sla_min * 0.5:
                return self._make(
                    task_id, "TASK_DELAY", max(1, int(sla_min - elapsed_min)),
                    predicted_value={
        ...
        if projected_completion_min <= sla_min * self.TASK_DELAY_SLA_RATIO:
            return None
        probability = min(0.99, 0.4 + (projected_completion_min - sla_min)
                          / sla_min * 0.5)
        confidence = 0.75 if elapsed_min >= 5 else 0.4
        return self._make(
            task_id, "TASK_DELAY", max(1, int(projected_completion_min - elapsed_min)),
            predicted_value={
                "progress_pct": progress_pct,
                "elapsed_min": elapsed_min,
                "sla_min": sla_min,
    ...
    def predict_zone_congestion(self, station_id, current_occupancy, trend, capacity):
        ...
        if predicted_occupancy <= capacity:
            return None
        probability = min(0.99, 0.4 + (predicted_occupancy - capacity)
                          / max(1.0, float(capacity)) * 0.5)
        confidence = 0.7 if abs(float(trend)) > 0.1 else 0.4
        return self._make(
            station_id, "ZONE_CONGESTION", horizon,
            predicted_value={
    ...
    def predict_device_offline(self, device_id, last_seen_ts, packet_loss_pct):
        ...
        triggered = (packet_loss_pct >= self.DEVICE_OFFLINE_PACKET_LOSS
                     or silence_sec >= self.DEVICE_OFFLINE_SILENCE_SEC)
        if not triggered:
            return None
