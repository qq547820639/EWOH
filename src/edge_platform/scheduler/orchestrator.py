...
class Scheduler:
    ...
    def _get(self, request_id):
        ...
        if req is None:
            raise KeyError("调度请求不存在: %s" % request_id)
        return req
    ...
    def _build_candidate_ctx(self, cand, ctx):
        ...
        cand_ctx = {
            "expected_production_uplift": pstate.get(
                ...
                ctx.get("expected_production_uplift", 0.0)),
            "on_time_probability": pstate.get(
                "on_time_probability",
                ctx.get("on_time_probability", 0.0)),
            ...
            "is_changeover": bool(pstate.get(
                "is_changeover", ctx.get("is_changeover", False))),
    ...
    def propose(self, task, persons, devices, ctx=None):
        ...
        candidates = self.generator.generate(
            task, persons, devices, self.constraints, ctx)
    ...
    def promote_to_proposed(self, request_id):
        ...
        if req.status != SHADOW:
            raise ValueError("仅 SHADOW 状态可升级为 PROPOSED，当前状态：%s" % req.status)
        req.status = PROPOSED
        return req
    ...
    def execute(self, request_id):
        ...
        if req.status != CONFIRMED:
            ...
            return {
                "request_id": request_id,
                "executed": False,
                "reason": "未经确认不得自动执行（当前状态：%s）" % req.status,
                "ts": now_iso(),
            }
        record = {
            "request_id": request_id,
