...
def explain_candidate(candidate):
    ...
    if violations:
        ...
        for v in violations:
            ...
            reasons.append("硬约束拦截[%s]：%s" % (ctype, reason))
    ...
    if score is not None:
        reasons.append("综合评分 %.3f" % float(score))
    ...
    for key, label in contrib_pairs:
        if key in bd and bd[key] is not None:
            contrib_strs.append("%s %+ .3f" % (label, float(bd[key])))
    if contrib_strs:
    ...
    if body_load is not None:
        ...
        if float(body_load) > float(baseline) + 1e-9:
            reasons.append(
                "当前累计负荷 %.2f 高于个人基线 %.2f，建议安排低负荷工位" % (
                    float(body_load), float(baseline))
            )
        else:
            reasons.append("当前累计负荷 %.2f 处于可接受范围" % float(body_load))

    # 移动距离（真实计算值）
    td = bd.get("travel_distance")
    if td is not None:
        reasons.append("到达工位移动距离 %.1f 米" % float(td))

    # 安全风险（真实计算值）
    sr = bd.get("safety_risk")
    if sr is not None and float(sr) > 1e-9:
        reasons.append("近期安全风险评分 %.2f，需关注" % float(sr))
...
def explain_plan(plan):
    ...
    reasons.append("方案共 %d 个候选，其中 %d 个通过硬约束、%d 个被拦截" % (
        len(candidates), len(passed), len(failed)))

    if passed:
        top = max(passed, key=lambda c: (getattr(c, "score", None)
                                         if getattr(c, "score", None) is not None
                                         else float("-inf")))
        if getattr(top, "score", None) is not None:
            reasons.append("推荐候选：人员 %s + 设备 %s，综合评分 %.3f" % (
                getattr(top, "person_id", ""),
                getattr(top, "device_id", ""),
                float(top.score)))
        # 附推荐候选的逐项理由
    ...
    if failed:
        ...
        summary = "、".join("%s×%d" % (k, v) for k, v in sorted(ctypes.items()))
        reasons.append("被拦截候选主要原因：%s" % summary)
