...
from .orchestrator import (
    SHADOW, CONFIRMED, REJECTED, EXECUTED,
)
...
def _normalize_metrics(raw):
    ...
    for canonical, aliases in (
        (_METRIC_PRODUCTION,
         ("production", "actual_production", "production_score")),
        (_METRIC_BODY_LOAD,
         ("body_load", "actual_body_load", "current_load")),
    ):
        for key in aliases:
    ...
    for k in ("on_time", "on_time_probability", "on_time_score",
              "safety_risk", "travel_distance"):
...
def _judge_improved(actual, predicted):
    ...
    无法判定（缺关键指标）时返回 None。
    """
    has_production = (_METRIC_PRODUCTION in actual
                      and _METRIC_PRODUCTION in predicted)
    has_body_load = (_METRIC_BODY_LOAD in actual
                     and _METRIC_BODY_LOAD in predicted)
    if not (has_production or has_body_load):
        return None
    improved = False
    if (has_production
            and actual[_METRIC_PRODUCTION] >= predicted[_METRIC_PRODUCTION]):
        improved = True
    if (not improved
            and has_body_load
            and actual[_METRIC_BODY_LOAD] <= predicted[_METRIC_BODY_LOAD]):
        improved = True
    return improved
...
class LearningLoop:
    ...
    def compute_stats(self, period_start=None, period_end=None):
        ...
        if start_ms is not None or end_ms is not None:
            filtered = [o for o in outcomes
                        if self._in_period(o.proposed_at, start_ms, end_ms)]
        else:
        ...
        improvement_rate = ((adopted_with_improvement / adopted_count)
                           if adopted_count else 0.0)
        meets_target = (adoption_rate >= _ADOPTION_RATE_TARGET
                        and improvement_rate > 0.0)
    ...
    def _calibrations_from_stats(self, stats):
        ...
        if _METRIC_PRODUCTION in avg and avg[_METRIC_PRODUCTION] < 0:
            ...
            suggestions.append(CalibrationSuggestion(
                ...
                reason=("实际产量持续低于预测（平均偏差 %.4f），"
                        "建议降低产量权重避免高估产能" % avg[_METRIC_PRODUCTION]),
                evidence={
        ...
        if _METRIC_BODY_LOAD in avg and avg[_METRIC_BODY_LOAD] > 0:
            ...
            suggestions.append(CalibrationSuggestion(
                ...
                current_value=cur,
                suggested_value=suggested,
                reason=("实际负荷持续高于预测（平均偏差 %.4f），"
                        "建议提升负荷权重以更谨慎排程" % avg[_METRIC_BODY_LOAD]),
                evidence={
    ...
    def export_period_report(self, period_start, period_end):
        ...
        stats = self.compute_stats(period_start=period_start,
        ...
        start_ms = ts_to_ms(period_start) if period_start else None
        end_ms = ts_to_ms(period_end) if period_end else None
        in_period = [o for o in self._outcomes
                     if self._in_period(o.proposed_at, start_ms, end_ms)]

        def _max_dev(o):
            if not o.metric_deltas:
                return 0.0
            return max((abs(float(v)) for v in o.metric_deltas.values()),
                       default=0.0)
