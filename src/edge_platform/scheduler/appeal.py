...
_VALID_RESOLUTION_ACTIONS = {
    ACT_ACKNOWLEDGE, ACT_CORRECT_DATA, ACT_CANCEL_DECISION, ACT_ESCALATE,
}
...
class AppealChannel:
    ...
    def _normalize_appeal_type(appeal_type):
        ...
        if isinstance(appeal_type, str) and appeal_type in AppealType._value2member_map_:
            return appeal_type
        raise ValueError("未知申诉类型: %r（合法取值：%s）" % (
            appeal_type,
            ",".join(t.value for t in AppealType),
        ))
    ...
    def submit(self, appellant_id, appeal_type, target_ref, description,
               evidence_refs=None):
        ...
        self._log(
            "submit", appellant_id, appeal_id,
            details="type=%s target=%s/%s" % (
                atype_val, target_ref.get("kind"), target_ref.get("id")),
        )
        return rec
    ...
    def acknowledge(self, appeal_id, reviewer_id):
        ...
        if rec.status != PENDING:
            raise ValueError("仅 PENDING 状态可认领，当前状态：%s" % rec.status)
    ...
    def resolve(self, appeal_id, reviewer_id, resolution, resolution_action):
        ...
        if rec.status not in (PENDING, REVIEWING):
            raise ValueError("仅 PENDING/REVIEWING 状态可处理，当前状态：%s" % rec.status)
        ...
        if resolution_action not in _VALID_RESOLUTION_ACTIONS:
            raise ValueError("未知处理动作: %r（合法取值：%s）" % (
                resolution_action, ",".join(sorted(_VALID_RESOLUTION_ACTIONS)),
            ))
        rec.status = RESOLVED
        rec.resolved_by = reviewer_id
        ...
        self._log(
            "resolve", reviewer_id, appeal_id,
            details="action=%s resolution=%s" % (resolution_action, rec.resolution),
        )
        return rec
    ...
    def reject(self, appeal_id, reviewer_id, reason):
        ...
        if rec.status in (RESOLVED, REJECTED):
            raise ValueError("已终态申诉不可驳回，当前状态：%s" % rec.status)
        rec.status = REJECTED
        rec.resolved_by = reviewer_id
        rec.resolution = str(reason or "").strip()
        rec.resolved_at = now_iso()
        self._log("reject", reviewer_id, appeal_id, details="reason=%s" % rec.resolution)
        return rec
    ...
    def list_by_target(self, target_kind, target_id):
        ...
        for r in self._appeals.values():
            if r.target_ref.get("kind") == target_kind and \
                    r.target_ref.get("id") == target_id:
                out.append(r)
        return out
    ...
    def stats(self):
        ...
        for r in self._appeals.values():
            ...
            if r.resolution_action:
                by_action[r.resolution_action] = \
                    by_action.get(r.resolution_action, 0) + 1
        return AppealStats(
