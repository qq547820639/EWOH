...
## W3：因果执行控制台 UX 深化
- [x] Task W3.1: 默认首页优先回答当前 Gate/阻塞任务/最长等待/过期证据/待人类决策风险/下一最优行动/过载项
- [x] Task W3.2: 响应式侧边导航 + 命令面板；全局搜索、保存视图、最近访问、URL 深链接
- [x] Task W3.3: 预设视图（我的待办/阻塞项/待批准/证据过期/资源冲突）
- [x] Task W3.4: DAG 平移缩放/自动聚焦关键路径/阶段折叠/渐进加载/节点搜索/上下游追踪/异常回流高亮/布局保存
- [x] Task W3.5: 图节点可访问列表替代视图；证据抽屉（commit/PR/测试命令/环境/日志/截图/校验和/验证人/有效期）
- [x] Task W3.6: 批量 Gate 操作先显示影响范围/缺失证据/不可执行原因；高风险操作支持条件批准/驳回/撤销/回滚点
- [x] Task W3.7: Agent 页面（负载/失败率/等待时间/工具权限/预算/最近交接）；资源页面（锁持有者/到期时间/等待队列/安全释放）
- [x] Task W3.8: 键盘快捷键（不得绕过审批）；大规模 Work Graph 性能基准
- [x] Task W3.9: 输出 W3 波次报告（前后截图、关键 Diff、测试命令与结果、Gate 变化）
