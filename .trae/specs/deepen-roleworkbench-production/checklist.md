...
- [ ] `.limit(5000)` 全表内存读取已移除（6 处）
- [ ] 工作台核心表具备 `org_id` 列与索引，查询强制包含 org_id
...
- [ ] 并发查询有超时/AbortSignal 取消/部分失败/慢查询日志 tracing/数据新鲜度时间戳
- [ ] 第 5001 条以后仍可查询；游标无重复无遗漏；跨组织越权测试通过
...
## C4 保存视图持久化
- [ ] `saved_views` 表 + migration 已实现（含全部要求字段与软删除）
- [x] 创建/重命名/复制/更新/删除；默认视图唯一性；乐观并发；schema 校验
- [x] 跨设备/重新登录恢复；多实例一致性；组织隔离 + owner 权限
- [ ] 旧 localStorage 一次性迁移后清理；schemaVersion 升级；冲突/迁移失败提示
- [x] 内存实现仅作显式 test adapter，生产配置不可误用
- [ ] 服务重启后视图仍存在；跨组织越权测试通过
