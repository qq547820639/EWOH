...
const PROBE_SUFFIX = randomUUID().slice(0, 8);
const ORG_A = randomUUID();
const ORG_B = randomUUID();
const ORG_B_CHILD = randomUUID();
const USERNAME = `ewoh_security_probe_${PROBE_SUFFIX}`;
...
async function main() {
  ...
  try {
    ...
    await owner`
      ...
      values
        (${ORG_A}::uuid, ${ORG_A}::uuid, 'Security Org A ' || ${PROBE_SUFFIX}, 'factory', 'active'),
        (${ORG_B}::uuid, ${ORG_B}::uuid, 'Security Org B ' || ${PROBE_SUFFIX}, 'factory', 'active')
    `;
    await owner`
    ...
    const directOrgs = await runtime`
      select * from public.ewoh_organization where id = ${ORG_A}::uuid
    `;
    assert(
      directOrgs.length === 0,
      'direct organization table read without GUC leaked the probe org',
    );
    const orgs = await runtime`select * from public.ewoh_find_org(${ORG_A}::uuid)`;
    assert(
      orgs.length === 1 && String(orgs[0].org_id) === ORG_A,
      'SECURITY DEFINER organization lookup failed',
    );
    evidence.org_lookup = 'direct org table read denied; SECURITY DEFINER org lookup resolves tenant';
