...
from .vision_adapter import project_to_floor, skeleton_to_posture
...
class PoseFusion:
    ...
    def fuse(self, person_id, uwb_sample, vision_det, exo_imu, station_id_hint, task_ctx):
        ...
        if vision_det is not None:
            ...
            if camera_pose is not None:
                proj = project_to_floor(vision_det.bbox_xyxy, camera_pose, camera_height_m, fov_v_deg)
                if proj is not None:
                    vx, vy, vconf = proj
                    # 视觉置信度取反投影质量与检测置信度的较小值（保守）
                    det_conf = max(0.0, min(1.0, float(vision_det.confidence)))
                    vis_conf = min(vconf, det_conf)
                    vis_pos = Pose(x=vx, y=vy, z=0.0, source="vision", confidence=vis_conf)
        ...
        if uwb_pos is not None and vis_pos is not None:
            ...
            if conflict is not None:
                ...
                if w_sum <= 0.0:
                    ...
                    unified_pose = Pose(x=fx, y=fy, z=fz, source="uwb+vision", confidence=base_conf)
