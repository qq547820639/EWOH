...
--------
  ...
  python src/edge_platform/scripts/record_raw_frames.py --port 9001 \
    --max-frames 1000 --duration-sec 600 --session-id pilot-20260730
"""
...
TYPE_NAMES = {0x01: "HEARTBEAT", 0x02: "IDENT", 0x10: "TELEMETRY", 0x11: "BACKFILL", 0x20: "FAULT"}
...
class FrameRecorder:
    ...
    def save_frame(self, device_id, frame, raw_bytes):
        ...
        with self._lock:
            ...
            fname = f"{time_part}_{int(millis):03}_{int(seq):06}.bin"
...
class DeviceHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        device_id = "unknown"
        buf = b""
        self.server.log(f"{peer} 设备连接")  # type: ignore
        try:
            ...
            self.server.log(f"{peer} 设备断开 (device_id={device_id})")  # type: ignore
...
def main(argv=None):
    ...
    log(f"录制会话启动: session_id={session_id}")
    log(f"输出目录: {os.path.abspath(session_dir)}")
    log(
        "监听: {}:{}  协议: {}（来源: {}）".format(
            args.host,
            args.port,
    ...
    log(
        "max_frames={}  duration_sec={}".format(
            args.max_frames if args.max_frames else "无限", args.duration_sec if args.duration_sec else "无限"
        )
    )
    ...
    try:
        ...
        log(f"无法监听 {args.host}:{int(args.port)}：{e}")
        return 2
    ...
    try:
        while not stop_event.is_set():
            if args.max_frames and recorder.total_frames >= args.max_frames:
                log(f"已达 max_frames={int(args.max_frames)}，停止录制")
                break
            if args.duration_sec and (time.time() - start_wall) >= args.duration_sec:
                log(f"已达 duration_sec={args.duration_sec}s，停止录制")
                break
            time.sleep(0.2)
    finally:
        stop_event.set()
        ...
        log(f"录制结束: total_frames={int(manifest['total_frames'])} device_ids={manifest['device_ids']}")
        log(f"manifest: {os.path.abspath(os.path.join(session_dir, 'manifest.json'))}")
    return 0
