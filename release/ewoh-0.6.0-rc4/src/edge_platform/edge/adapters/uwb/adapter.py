...
class UWBAdapter(BaseAdapter):
    ...
    def __init__(self, device_id, beacons, tags, source_type="real", model="UWB-GENERIC", firmware_version=""):
...
class SimulatedUWBAdapter(UWBAdapter):
    ...
    def __init__(self, device_id, tag_id, person_id, path, beacons=None, hz=2.0, source_type="simulated"):
        beacons = list(beacons) if beacons else []
        tags = [UWBTag(tag_id=tag_id, person_id=person_id)]
        super().__init__(device_id, beacons=beacons, tags=tags, source_type=source_type, model="UWB-SIM")
        self.tag_id = tag_id
