...
def encode_frame(frame_type, seq, ts_ms, payload):
    ...
    if frame_type not in UPLINK_TYPES:
        ...
        raise ValueError(f"拒绝编码非上行帧 TYPE=0x{frame_type & 0xFF:02X} ({name})：平台不实现任何向设备写入的路径")
    payload = bytes(payload or b"")
    if len(payload) > MAX_PAYLOAD_LEN:
        raise ValueError(f"payload 超过 1B LEN 上限: {len(payload)} B")
    body = (
...
def encode_ident(device_id, seq=0, ts_ms=0, firmware_version=DEFAULT_FIRMWARE, hardware_version=DEFAULT_HARDWARE):
...
def encode_heartbeat(battery_pct=100, status=0, seq=0, ts_ms=0, firmware_version=DEFAULT_FIRMWARE):
...
def encode_telemetry_payload(
    ...
    return struct.pack(
        ...
        _phys_to_i16(ax_mg, 1),
        _phys_to_i16(ay_mg, 1),
        _phys_to_i16(az_mg, 1),
...
def encode_telemetry(seq=0, ts_ms=0, **fields):
    ...
    return encode_frame(protocol.TYPE_TELEMETRY, seq, ts_ms, encode_telemetry_payload(**fields))


def encode_fault(fault_code, detail=0, seq=0, ts_ms=0):
    """FAULT(0x20)：code u8 + detail u8；code=0x00 表示故障恢复。"""
    return encode_frame(protocol.TYPE_FAULT, seq, ts_ms, bytes([int(fault_code) & 0xFF, int(detail) & 0xFF]))
...
def encode_backfill(entries, seq=0, ts_ms=0):
    ...
    if len(entries) > MAX_BACKFILL_ENTRIES_PER_FRAME:
        raise ValueError(f"单个 BACKFILL 帧最多 {int(MAX_BACKFILL_ENTRIES_PER_FRAME)} 条，实际 {len(entries)} 条")
...
def corrupt_crc(frame_bytes, offset=16):
    ...
    if len(data) <= offset:
        raise ValueError(f"帧长度 {len(data)} 不足以在偏移 {int(offset)} 处注入错误")
    data[offset] ^= 0xFF
    return bytes(data)
...
__all__ = [
    ...
    "DOWNLINK_TYPES",
    "FORBIDDEN_WRITE_CAPABILITIES",
    "MAX_BACKFILL_ENTRIES_PER_FRAME",
    "MAX_BACKFILL_CACHE",
    "encode_frame",
    "encode_ident",
    "encode_heartbeat",
