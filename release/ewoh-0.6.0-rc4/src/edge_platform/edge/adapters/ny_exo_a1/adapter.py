...
class NyExoA1Adapter(BaseAdapter):
    ...
    def __init__(
        ...
        frame_sink=None,
        keep_raw=False,
        raw_ring_size=DEFAULT_RAW_RING_SIZE,
    ...
    def _handle_frame(self, frame, raw=b""):
        ...
        if frame["type"] == protocol.TYPE_IDENT:
            ...
            if new_fw and old_fw and new_fw != old_fw:
                ...
                return [
                    (
                        self._firmware_upgrade_frame(self._firmware_upgraded),
    ...
    def to_unified(self, telemetry, ts_ms, raw_bytes=b"", quality_reasons=None):
        ...
        non_numeric_fields = [
            name
            for name, val in (("pitch_deg", pitch), ("roll_deg", roll), ("torque_nm", torque), ("battery_pct", battery))
        ...
        if non_numeric:
            quality_status, confidence, reason = ("invalid", 0.0, "non_numeric:" + ",".join(non_numeric_fields))
        elif out_of_range:
            ...
            quality_status, confidence, reason = ("degraded", 0.5, ",".join(upstream_reasons))
        else:
...
def frames_from_bytes(
    raw, device_id="EXO-UNKNOWN", source_type="real", worker_id=None, tz_offset_hours=DEFAULT_TZ_OFFSET_HOURS
):
    """便捷函数：一段原始字节流 → [UnifiedExoFrame, ...]（离线回放/fixture 测试用）。

    仅解析，不启动适配器线程；坏帧按协议约定丢弃。
    """
    adapter = NyExoA1Adapter(device_id, source_type=source_type, worker_id=worker_id, tz_offset_hours=tz_offset_hours)
    adapter.feed(raw)
