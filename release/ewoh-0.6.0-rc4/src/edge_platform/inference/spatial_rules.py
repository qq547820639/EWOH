...
from edge_platform.inference import new_id as inf_new_id
...
class RuleBase:
    ...
    def _finding(self, person_id, device_id, station_id, message, evidence, triggered_at, confidence=1.0):
...
class PostureThresholdRule(RuleBase):
    ...
    def evaluate(self, ctx) -> Optional[RuleFinding]:
        ...
        if cond:
            ...
            if not self._open.get(person_id) and now_ms - self._since[person_id] >= sustained_ms:
                ...
                return self._finding(
                    ...
                    message="躯干前倾角 {:.1f}° 超阈值 {:.1f}° 持续 ≥{}s".format(
...
class HighLoadDurationRule(RuleBase):
    ...
    def evaluate(self, ctx) -> Optional[RuleFinding]:
        ...
        cond = (assist is not None and assist > a_thr) or (torque is not None and torque > t_thr)
        person_id = _get(fs, "person_id")
        sustained_ms = self.config["sustained_sec"] * 1000
        if cond:
            if person_id not in self._since:
            ...
            if not self._open.get(person_id) and now_ms - self._since[person_id] >= sustained_ms:
                ...
                return self._finding(
                    ...
                    device_id=_get(fs, "device_id"),
                    station_id=_get(fs, "station_id"),
                    message="高负荷持续 ≥{}s（assist={} torque={}Nm）".format(
                        self.config["sustained_sec"], _fmt(assist), _fmt(torque, "%.1f")
                    ),
                    evidence={
                        "assist_level": assist,
                        "torque_nm": torque,
        ...
        if count > max_count:
            if not self._open.get(person_id):
                ...
                return self._finding(
                    ...
                    device_id=_get(fs, "device_id"),
                    station_id=_get(fs, "station_id"),
                    message=f"近 {self.config['window_sec']}s 内高负荷动作 {int(count)} 次超过上限 {int(max_count)} 次",
                    evidence={
        ...
        if 0 < predicted_min <= horizon:
            if not self._open.get(device_id):
                ...
                return self._finding(
                    ...
                    station_id=_get(fs, "station_id"),
                    message=(
                        f"预计 {predicted_min:.1f} 分钟后电量降至 {low:.0f}%（当前 {bat:.1f}%，放电 {drain:.2f}%/min）"
                    ),
                    evidence={
                        "battery_percent": bat,
                        "low_threshold": low,
        ...
        if gap_sec >= self.config["offline_sec"]:
            if not self._open.get(device_id):
                ...
                return self._finding(
                    ...
                    device_id=device_id,
                    station_id=_get(fs, "station_id"),
                    message=f"设备已失联 {gap_sec:.0f} 秒（阈值 {self.config['offline_sec']} 秒）",
                    evidence={
...
class StationDwellRule(RuleBase):
    ...
    def evaluate(self, ctx) -> Optional[RuleFinding]:
        ...
        if enter_ms is None:
            return None
        person_id = _get(fs, "person_id")
        expected = _num(_get(fs, "expected_dwell_sec"), self.config["default_expected_dwell_sec"])
        ...
        if dwell_sec >= expected + slack:
            ...
            if not self._open.get(key):
                ...
                return self._finding(
                    ...
                    device_id=_get(fs, "device_id"),
                    station_id=station_id,
                    message=f"工位 {station_id} 停留 {dwell_sec:.0f} 秒超出预期 {expected:.0f} 秒（容差 {slack} 秒）",
                    evidence={
                        "station_id": station_id,
                ...
                if gap >= self.config["start_overdue_sec"]:
                    ...
                    if not self._open.get(key):
                        ...
                        return self._finding(
                            ...
                            device_id=device_id,
                            station_id=station_id,
                            message=f"任务 {task_id} 未按期开工（超期 {gap:.0f} 秒）",
                            evidence={
                                "task_id": task_id,
                                "kind": "start_overdue",
                    ...
                    if gap >= 0:
                        ...
                        if not self._open.get(key):
                            ...
                            return self._finding(
                                ...
                                device_id=device_id,
                                station_id=station_id,
                                message=f"任务 {task_id} 未按期完工（超期 {gap:.0f} 秒）",
                                evidence={
                                    "task_id": task_id,
            ...
            if inside:
                if not self._open.get(key):
                    ...
                    return self._finding(
                        ...
                        device_id=_get(fs, "device_id"),
                        station_id=_get(fs, "station_id"),
                        message=f"人员进入禁区 {zone_id}（{label}）",
                        evidence={
                            "zone_id": zone_id,
                            "zone_label": label,
    ...
    def evaluate(self, ctx) -> Optional[RuleFinding]:
        ...
        for c in conflicts[: self.config["max_per_call"]]:
            ...
            return self._finding(
                ...
                message=f"传感器冲突：{ctype}（UWB 工位 {uwb_st} vs 视觉工位 {vis_st}）",
                evidence={
                    "conflict_type": ctype,
                    "uwb_station": uwb_st,
        ...
        if st["value"] >= threshold:
            if not self._open.get(person_id):
                ...
                return self._finding(
                    ...
                    device_id=_get(fs, "device_id"),
                    station_id=_get(fs, "station_id"),
                    message=f"累计负荷积分 {st['value']:.1f} 超阈值 {threshold:.1f}",
                    evidence={
