...
class RuleEngine:
    ...
    def _track(self, st_key, cond_enter, ts_ms, dur_sec, code, dev, person, src, condition, cond_exit=None):
        ...
        if cond_enter:
            ...
            if (dur_sec == 0 or ts_ms - st[st_key] >= dur_sec * 1000) and key not in self._open:
    ...
    def on_telemetry(self, msg):
        ...
        drafts += self._track(
            ...
            person,
            src,
            f"pitch_deg>{self.cfg['bend_pitch_deg']} 持续>={self.cfg['bend_sec']}s",
            cond_exit=bend_exit,
        )

        torque = _f(tel.get("torque_nm"))
        assist = _f(tel.get("assist_level"))
        load_enter = (torque is not None and torque > self.cfg["load_torque_nm"]) or (
            assist is not None and assist > self.cfg["load_assist"]
        )
        load_exit = (torque is not None and torque > self.cfg["load_torque_exit_nm"]) or (
            assist is not None and assist > self.cfg["load_assist_exit"]
        )
        drafts += self._track(
            "load_since",
            load_enter,
            ...
            person,
            src,
            "torque_nm>{} 或 assist_level>{} 持续>={}s".format(
        ...
        drafts += self._track(
            ...
            cond_deg,
            ts_ms,
            self.cfg["degraded_sec"],
            "SENSOR_DEGRADED",
            dev,
            person,
            src,
            f"quality.status!=good 持续>={self.cfg['degraded_sec']}s",
        )
    ...
    def on_inference(self, res):
        ...
        drafts += self._track(
            "action_anomaly_since",
            cond,
            ts_ms,
            self.cfg["action_anomaly_sec"],
            "ACTION_ANOMALY_LOW_QUALITY",
            dev,
            person,
            src,
    ...
    def on_offline(self, device_id, ts):
        ...
        if key in self._open or not self._cooldown_ok(key, ts_ms):
            return None
        d = self._draft("DEVICE_OFFLINE", device_id, None, ts_ms, "real", "心跳超时/通信中断")
        self._open[key] = d
        self._fired[key] = ts_ms
        return d
