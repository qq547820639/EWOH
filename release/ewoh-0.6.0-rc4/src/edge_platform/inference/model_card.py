...
def build_action_classifier_card(model, metrics, dataset_manifest, created_by="system"):
    ...
    return ModelCard(
        ...
        dataset_split=dict(dataset_manifest) if isinstance(dataset_manifest, dict) else {"raw": dataset_manifest},
...
def assert_non_medical(card):
    ...
    if not has_non_medical:
        raise AssertionError(f"ethical_notes 必须含「非医学诊断」: {notes!r}")

    desc = str(getattr(card, "description", ""))
    for forbidden in ("患病", "健康异常"):
        if forbidden in desc:
            raise AssertionError(f"description 不得含「{forbidden}」字样（非医学诊断边界）")
