...
from edge_platform.inference.model_card import (
    ...
    DEFAULT_LIMITATIONS,
    DEFAULT_OUT_OF_SCOPE_USES,
    ModelCard,
    assert_non_medical,
    build_action_classifier_card,
)


def _mk_sample(pid, label="stand", i=0):
    return {"person_id": pid, "label": label, "session_id": f"SES-{pid}", "idx": i}
...
class SplitByPersonTest(unittest.TestCase):
    def test_basic_split_counts_and_disjoint(self):
        persons = [f"P{int(i):02}" for i in range(20)]
    ...
    def test_reproducible_with_same_seed(self):
        samples = _mk_samples([f"P{int(i):02}" for i in range(15)], per_person=3)
        s1 = split_by_person(samples, seed=7)
        s2 = split_by_person(samples, seed=7)
        # 同 seed → 同样的人员归属
        for k in ("train", "val", "test"):
            self.assertEqual([s["person_id"] for s in s1[k]], [s["person_id"] for s in s2[k]])
        # 不同 seed → 人员归属应不同（极大概率；此处校验 train 人员集合不同）
        s3 = split_by_person(samples, seed=99)
        self.assertNotEqual({s["person_id"] for s in s1["train"]}, {s["person_id"] for s in s3["train"]})
    ...
    def test_disjoint_assertion_invariant_holds(self):
        ...
        samples = _mk_samples([f"P{int(i):02}" for i in range(12)], per_person=2)
        for seed in range(10):
            splits = split_by_person(samples, seed=seed)
            ok, overlap = verify_no_person_leak(splits)
            self.assertTrue(ok, f"seed={int(seed)} 出现人员泄漏: {overlap!r}")
...
class ManifestRoundTripTest(unittest.TestCase):
    ...
    def test_write_read_roundtrip(self):
        samples = _mk_samples([f"P{int(i):02}" for i in range(10)], per_person=3)
        ...
        self.assertEqual(tr | va | te, {f"P{int(i):02}" for i in range(10)})
...
class ModelCardTest(unittest.TestCase):
    ...
    def test_build_action_classifier_card_fields_complete(self):
        metrics = {
            "macro_f1": 0.9,
            "per_class_f1": {"stand": 0.93},
            "high_risk_recall": 0.95,
            "false_positive_rate": 0.05,
        }
        manifest = {"version": "0.2", "person_ids": {"train": ["P1"], "val": [], "test": []}}
        card = build_action_classifier_card(_mk_model("0.2.0"), metrics, manifest, created_by="trainer")
        # 字段完整性
        ...
        self.assertEqual(set(card.label_set) - {"unknown"}, {"stand", "walk", "bend", "lift"})
