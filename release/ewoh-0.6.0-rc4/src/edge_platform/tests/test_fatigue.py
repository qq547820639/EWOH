...
def sample(minute, action, load, assist=0.0, duration=60.0):
    ...
    return LoadSample(ts=ts(minute), action=action, load_level=load, assist_level=assist, duration_sec=duration)
...
class LowLoadShiftTest(unittest.TestCase):
    def test_low_load_is_low_and_stable(self):
        ...
        for i in range(30):
            scorer.ingest("P1", sample(i, "walk" if i % 2 == 0 else "stand", load=0.2, assist=0.1))
...
class HighLoadBendingLiftsTest(unittest.TestCase):
    def _build(self):
        ...
        baseline_samples = [sample(i, "walk", load=0.35, assist=0.3) for i in range(10)]
        baseline_samples += [sample(10 + i, "lift", load=0.5, assist=0.3) for i in range(2)]  # 基线搬运 2 次
        ...
        for i in range(n):
            ...
            if i % 3 == 0:
                ...
                action = "carry"
            scorer.ingest("P2", sample(i, action, load=load, assist=0.5, duration=60.0))
        return scorer
...
class BaselineDeviationTest(unittest.TestCase):
    def test_deviation_above_baseline(self):
        ...
        scorer.update_baseline("P3", [sample(i, "walk", load=0.5, duration=60.0) for i in range(10)])
...
class FormatReportTest(unittest.TestCase):
    ...
    def test_format_empty_causes(self):
        report = FatigueReport(
            ...
            current_load_score=0.1,
            trend_30min="STABLE",
            trend_per_min=0.0,
            main_causes=[],
            recommendation="负荷正常，保持当前节奏",
            individual_baseline=0.0,
...
class IsMedicalInvariantTest(unittest.TestCase):
    def test_is_medical_always_false_across_scenarios(self):
        ...
        for i in range(20):
            s2.ingest("B", sample(i, "bend" if i % 2 else "lift", load=0.6 + 0.3 * (i / 19), assist=0.4))
...
class RecoveryTimeTest(unittest.TestCase):
    ...
    def test_high_load_report_recovery_in_recommendation(self):
        ...
        # 高负荷：弯腰 + 搬运交替，负荷持续高位
        for i in range(20):
            scorer.ingest("R", sample(i, "bend" if i % 2 == 0 else "lift", load=0.9, assist=0.3))
...
class ComponentsTest(unittest.TestCase):
    def test_shift_cumulative_reflects_load_integral(self):
        ...
        for i in range(120):
            scorer.ingest("S", sample(i, "lift", load=0.6, assist=0.3, duration=60.0))
