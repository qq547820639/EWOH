...
if (!e2eConfig) {
  ...
  describe('EWOH HTTP + PostgreSQL E2E (SP-01..SP-08)', () => {
    ...
    it('serves the work orchestration control plane with role gating', async () => {
      const admin = await login(
        baseUrl,
        fixture!.globalAdminA.username,
        fixture!.globalAdminA.password,
      ...
      const graph = await apiRequest<{
        schema: string;
        summary: { itemCount: number; gateCount: number };
      }>(baseUrl, '/api/work/graph', {
        headers: jsonHeaders(adminToken),
      });
      expect(graph.status).toBe(200);
      expect(graph.body.schema).toBe('ewoh:///work-graph/v1');
      expect(graph.body.summary.itemCount).toBeGreaterThan(0);
      expect(graph.body.summary.gateCount).toBeGreaterThanOrEqual(14);

      const gates = await apiRequest<
        Array<{ gateId: string; calculatedStatus: string }>
      >(baseUrl, '/api/work/gates', {
        headers: jsonHeaders(adminToken),
      });
      expect(gates.status).toBe(200);
      expect(
        gates.body.find((gate) => gate.gateId === 'G10')?.calculatedStatus,
      ).toBe('requires_approval');

      const catalog = await apiRequest<{
        catalogId: string;
        assets: unknown[];
      }>(baseUrl, '/api/work/catalog', {
        headers: jsonHeaders(adminToken),
      });
      expect(catalog.status).toBe(200);
      expect(catalog.body.catalogId).toBe('ewoh-final6-catalog');
      expect(catalog.body.assets.length).toBeGreaterThanOrEqual(8);

      const gitSync = await apiRequest<{
        schema: string;
        itemCount: number;
        status: string;
      }>(baseUrl, '/api/work/git-sync', {
        headers: jsonHeaders(adminToken),
      });
      expect(gitSync.status).toBe(200);
      ...
      expect(siteReadiness.status).toBe(200);
      expect(siteReadiness.body.length).toBeGreaterThanOrEqual(2);
      expect(siteReadiness.body.some((report) => report.ready)).toBe(true);
      expect(siteReadiness.body.some((report) => !report.ready)).toBe(true);

      const filteredItems = await apiRequest<
        Array<{ id: string; title: string }>
      >(baseUrl, '/api/work/items?q=T-001&limit=5', {
        headers: jsonHeaders(adminToken),
      });
      expect(filteredItems.status).toBe(200);
      expect(filteredItems.body.length).toBeLessThanOrEqual(5);
      expect(
        filteredItems.body.every((item) => item.id.includes('T-001')),
      ).toBe(true);

      const limitedEvidence = await apiRequest<unknown[]>(
        baseUrl,
        '/api/work/evidence?limit=3',
        { headers: jsonHeaders(adminToken) },
      );
      expect(limitedEvidence.status).toBe(200);
      expect(limitedEvidence.body.length).toBeLessThanOrEqual(3);

      const evidenceList = await apiRequest<Array<{ evidenceId: string }>>(
        baseUrl,
        '/api/work/evidence?limit=1',
        { headers: jsonHeaders(adminToken) },
      );
      expect(evidenceList.status).toBe(200);
      expect(evidenceList.body.length).toBe(1);
      const preview = await apiRequest<{
        ...
        baseUrl,
        `/api/work/evidence/${evidenceList.body[0].evidenceId}/content?limit=5`,
        { headers: jsonHeaders(adminToken) },
      );
      expect(preview.status).toBe(200);
      expect(preview.body.evidenceId).toBe(evidenceList.body[0].evidenceId);
      expect(preview.body.lines).toBeGreaterThan(0);
      expect(preview.body.content.length).toBeGreaterThan(0);

      const write = await apiRequest(baseUrl, '/api/work/handoffs', {
        method: 'POST',
        headers: jsonHeaders(adminToken),
        body: JSON.stringify({
          fromActor: 'ORCH-05',
          toActor: 'VAL-61',
          scope: 'E2E write gate',
        }),
      });
      ...
      expect(viewer.status).toBe(201);
      const denied = await apiRequest(baseUrl, '/api/work/graph', {
        headers: jsonHeaders(viewer.body.accessToken),
      });
      expect(denied.status).toBe(403);
    });
