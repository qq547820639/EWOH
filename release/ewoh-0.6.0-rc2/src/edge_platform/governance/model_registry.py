...
class ModelRecord:
    ...
    def __post_init__(self):
        ...
        if self.model_type not in VALID_MODEL_TYPES:
            raise ValueError(f"未知模型类型: {self.model_type!r}")
...
class ModelRegistry:
    ...
    # ---- 审计 ----
    def _log(self, model_id, action, actor_id, audit_ref, from_status, to_status, detail=""):
        entry = {
            "log_id": new_id("MAUD"),
            "model_id": model_id,
            "action": action,
            ...
            "ts": now_iso(),
            "ref": audit_ref,
            "from_status": (from_status.value if isinstance(from_status, ModelStatus) else from_status),
            "to_status": (to_status.value if isinstance(to_status, ModelStatus) else to_status),
            "detail": detail,
        }
    ...
    def _was_active_or_shadow(self, model_id):
        ...
        for e in self.audit_trail:
            if e["model_id"] == model_id and e["to_status"] in ("ACTIVE", "SHADOW", "CANARY"):
                return True
        return False
    ...
    def register(self, model_record):
        ...
        self._log(
            ...
            detail=(
                ...
                f"data={model_record.data_version} "
                f"feature={model_record.feature_version} "
                f"threshold={model_record.threshold_version}"
            ),
        )
        return model_record
    ...
    def submit_for_review(self, model_id, submitter_id):
        ...
        entry = self._log(model_id, "submit_for_review", submitter_id, "", old, ModelStatus.REVIEWING)
    ...
    def approve_review(self, model_id, reviewer_id):
        ...
        entry = self._log(model_id, "approve_review", reviewer_id, "", old, ModelStatus.SHADOW)
        rec.audit_ref = entry["log_id"]
        return rec
    ...
    def start_controlled_validation(self, model_id):
        ...
        entry = self._log(model_id, "start_controlled_validation", "", "", old, ModelStatus.CONTROLLED_VALIDATION)
        rec.audit_ref = entry["log_id"]
        return rec
    ...
    def start_canary(self, model_id, canary_ratio=0.1):
        ...
        entry = self._log(
            model_id, "start_canary", "", "", old, ModelStatus.CANARY, detail=f"canary_ratio={canary_ratio}"
        )
        rec.audit_ref = entry["log_id"]
        return rec
    ...
    def promote_to_shadow(self, model_id, audit_ref):
        ...
        old = rec.status
        rec.status = ModelStatus.SHADOW
        entry = self._log(model_id, "promote_to_shadow", "", audit_ref, old, ModelStatus.SHADOW)
        rec.audit_ref = entry["log_id"]
        return rec
    ...
    def activate(self, model_id, approver_id):
        ...
        if rec.status is not ModelStatus.CANARY:
            raise ValueError(f"仅 CANARY 可激活为建议模式（需人工批准），当前状态: {rec.status.value}")
        ...
        if cur is not None and cur.model_id != model_id:
            ...
            self._log(cur.model_id, "retire(auto)", "", "", old, ModelStatus.RETIRED, detail=f"被 {model_id} 取代")
        old = rec.status
    ...
    def rollback(self, model_type, to_model_id, audit_ref):
        ...
        if target.model_type != model_type:
            raise ValueError(f"目标模型类型不匹配: {target.model_type} vs {model_type}")
        if not self._was_active_or_shadow(to_model_id):
            raise ValueError(f"仅可回滚到曾 ACTIVE/SHADOW 的版本: {to_model_id}")
        ...
        if cur is not None and cur.model_id != to_model_id:
            ...
            self._log(
                ...
                old,
                ModelStatus.RETIRED,
                detail=f"回滚让位 {to_model_id}",
            )
        old = target.status
        target.status = ModelStatus.ACTIVE
        target.activated_at = now_iso()
        entry = self._log(to_model_id, "rollback", "", audit_ref, old, ModelStatus.ACTIVE, detail="回滚到该版本")
