...
class RestoreTest(unittest.TestCase):
    ...
    def test_restore_backup_not_found(self):
        with self.assertRaises(FileNotFoundError):
            self.mgr.restore(os.path.join(self.tmp, "nope.db"), os.path.join(self.tmp, "out.db"))
...
class CLITest(unittest.TestCase):
    ...
    def test_cli_backup(self):
        rc = backup_db.main(["--action", "backup", "--db", self.db_path, "--output", self.output_dir])
    ...
    def test_cli_list_with_backups(self):
        backup_db.main(["--action", "backup", "--db", self.db_path, "--output", self.output_dir])
        rc = backup_db.main(["--action", "list", "--output", self.output_dir])
        self.assertEqual(rc, 0)
    ...
    def test_cli_restore(self):
        ...
        # 恢复到新路径
        target = os.path.join(self.tmp, "restored.db")
        rc = backup_db.main(["--action", "restore", "--backup", backup_file, "--db", target])
        self.assertEqual(rc, 0)
    ...
    def test_cli_backup_missing_db(self):
        rc = backup_db.main(
            ["--action", "backup", "--db", os.path.join(self.tmp, "nope.db"), "--output", self.output_dir]
        )
