...
from .model import ActionModel, ModelError, ModelRegistry
...
def load_dataset(ds_dir):
    ...
    for a, b in itertools.combinations(("train", "val", "test"), 2):
        ...
        if dup:
            raise SystemExit(f"人员泄漏：{sorted(dup)} 同时出现在 {a} 与 {b}")
...
def benchmark_latency(model, samples, n=BENCH_N):
    ...
    pool = [s["features"] for s in samples] or [
        {k: c[i] for i, k in enumerate(model.feature_names)} for c in model.centroids.values()
    ]
    ts = []
    for i in range(n):
...
def next_version(out_dir):
    ...
    if best is None:
        return "0.1.0"
    return f"{int(best[0])}.{int(best[1] + 1)}.{0}"
...
def write_eval_report(path, ctx):
    m = ctx["test_metrics"]
    lines = [
        f"# 动作分类模型评测报告 {ctx['model_id']}-v{ctx['version']}",
        "",
        f"- 评测时间：{ctx['created_at']}",
        f"- 数据集版本：{ctx['dataset_version']}（source_type={ctx['source_type']}）",
        "- unknown 阈值：min_confidence={:.2f}（val 选取），ambiguous_gap={:.2f}".format(
            ctx["thresholds"]["min_confidence"], ctx["thresholds"]["ambiguous_gap"]
        ),
        "",
        "## 人员独立性声明",
        "",
        "本评测严格按人员分组划分，train/val/test 三组人员无任何交集（加载时已断言，发现交集直接报错退出）：",
        f"- train（{len(ctx['persons']['train'])} 人）：{', '.join(ctx['persons']['train'])}",
        f"- val（{len(ctx['persons']['val'])} 人）：{', '.join(ctx['persons']['val'])}",
        f"- test（{len(ctx['persons']['test'])} 人）：{', '.join(ctx['persons']['test'])}",
        "",
        "## 总体指标（test，人员独立）",
        "",
        f"- Macro-F1：{m['macro_f1']:.4f}（内部目标 ≥ {TARGET_MACRO_F1:.2f}）",
        f"- unknown 率：{m['unknown_rate']:.4f}",
        f"- 样本数：{int(m['n_samples'])}",
        ("- 延迟基准（连续 {} 次 predict）：P50={:.3f} ms，P95={:.3f} ms").format(
    ...
    for c, v in m["per_class"].items():
        lines.append(f"| {c} | {v['precision']:.4f} | {v['recall']:.4f} | {v['f1']:.4f} | {int(v['support'])} |")
    ...
    for t, row in m["confusion"].items():
        lines.append(f"| {t} | {' | '.join(str(row[c]) for c in cols)} |")
    lines += ["", "## 结论", ""]
    if m["macro_f1"] >= TARGET_MACRO_F1:
        lines.append(
            "人员独立测试 Macro-F1={:.4f}，达到内部目标（≥{:.2f}），可进入发布评审。".format(
                m["macro_f1"], TARGET_MACRO_F1
            )
        )
    else:
        lines.append(
            "人员独立测试 Macro-F1={:.4f}，未达内部目标（≥{:.2f}）。"
            "建议如实缩小动作类别集合或补充受控数据后重训，"
            "不得通过包装数据/放宽口径宣称达标。".format(m["macro_f1"], TARGET_MACRO_F1)
        )
...
def write_model_card(path, ctx):
    ...
    per_class = "；".join(f"{c} R={v['recall']:.3f}" for c, v in m["per_class"].items())
    content = (
        "\n".join(
            [
                "# 模型卡：{model_id}",
                ...
                "- 输入传感器/字段：标准遥测的 pitch_deg/roll_deg、三轴 acceleration、"
                "三轴 angular_velocity、torque_nm、assist_level",
                ...
                "- 训练数据：来源 {source_type}；{n_persons} 名匿名受试者；受控采集会话"
                "（设备/固件/穿戴记录于 collection_session）；动作标签 {labels}；"
                "授权记录见采集会话 consent_id（本包不内嵌授权文本）",
                "- 数据划分：按人员分组 train {train_n} 人 / val {val_n} 人 / test {test_n} 人，三组无交集（已断言）",
                "- 指标：Macro-F1={macro_f1:.4f}（人员独立 test）；每类召回：{per_class}；"
                "unknown 率={unknown_rate:.4f}；延迟 P50={p50:.3f}ms / P95={p95:.3f}ms"
                "（{bench_n} 次）",
                "- 资源需求：纯 CPU、Python 标准库、内存 <50MB；无 GPU 依赖",
                "- 已知限制：最近质心线性可分性有限；对未受训人员存在泛化误差；"
                "invalid 占比>30% 的窗口直接 unknown；过渡动作与负荷极重片段易判 unknown",
        ...
        n_persons=ctx["n_persons"],
        train_n=len(ctx["persons"]["train"]),
        val_n=len(ctx["persons"]["val"]),
...
def run(argv=None):
    ...
    out_dir = os.path.join(args.out, f"action-classifier-v{version}")
    ...
    ctx = {
        ...
        "persons": persons,
        "n_persons": len(set(sum(persons.values(), []))),
    ...
    if args.register:
        ...
        reg.register(
            version,
            os.path.join(f"action-classifier-v{version}", "model.json"),
            {
                "model_id": model.model_id,
    ...
    print(
        ...
        "P95={:.3f}ms，val阈值={:.2f}（F1={}），输出 {}{}".format(
            model.model_id,
            version,
            test_metrics["macro_f1"],
            TARGET_MACRO_F1,
            test_metrics["unknown_rate"],
            latency["p95"],
            best_th,
