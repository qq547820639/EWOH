...
class ScenarioSimulator:
    ...
    def generate_plans(self, schedule_request, ctx, types=None):
        ...
        ctx = ctx or {}
        if types is None:
            types = [PlanType.KEEP_CURRENT, PlanType.CAPACITY_FIRST, PlanType.SAFETY_BALANCED]
        # 归一化为字符串值
    ...
    def _minimal_adjust(self, ctx):
        ...
        for pid, a in assignment.items():
            ...
            load = float(pstate.get("current_load", 0.0)) + float(tstate.get("load_add", 0.0))
            if load > worst_load:
                worst_load = load
                worst_pid = pid
    ...
    def _capacity_first(self, ctx):
        ...
        person_ids = sorted(
            persons_state.keys(), key=lambda pid: float(persons_state[pid].get("speed_factor", 1.0)), reverse=True
        )
        station_items = []
        for sid, sstate in stations_state.items():
        ...
        non_faulty = [did for did, dstate in devices_state.items() if not dstate.get("faulty")]
        if not non_faulty:
    ...
    def _safety_balanced(self, ctx):
        ...
        # 人员按当前负荷升序（最空闲优先接活）
        person_ids = sorted(persons_state.keys(), key=lambda pid: float(persons_state[pid].get("current_load", 0.0)))
        # 工位按任务预期产量降序（高产出优先安排）
        station_items = []
        for sid, sstate in stations_state.items():
            tid = sstate.get("task_id", "")
        ...
        healthy = [
            did
            for did, dstate in devices_state.items()
            if _predicted_battery(dstate, remaining_hours) >= battery_low_threshold
        ]
        if not healthy:
            healthy = [did for did, dstate in devices_state.items() if not dstate.get("faulty")]
        if not healthy:
            healthy = [""]
        healthy.sort(key=lambda did: _predicted_battery(devices_state.get(did, {}), remaining_hours), reverse=True)

        assignment = {}
        used_persons = set()
        person_loads = {pid: float(persons_state[pid].get("current_load", 0.0)) for pid in person_ids}
    ...
    def _equipment_emergency(self, ctx):
        ...
        assignment = {pid: dict(a) for pid, a in current.items()}
        dev_idx = 0
        for _pid, a in assignment.items():
            did = a.get("device_id", "")
            dstate = devices_state.get(did, {}) or {}
            need_swap = dstate.get("faulty") or _predicted_battery(dstate, remaining_hours) < battery_low_threshold
            if need_swap:
