...
def _ctx():
    ...
    return {
        ...
        "stations_state": {
            "s1": {"pose": Pose(x=10.0, y=0.0), "capacity": 2, "current_occupancy": 1, "task_id": "t1"},
            "s2": {"pose": Pose(x=0.0, y=10.0), "capacity": 2, "current_occupancy": 1, "task_id": "t2"},
            "s3": {"pose": Pose(x=2.0, y=0.0), "capacity": 2, "current_occupancy": 1, "task_id": "t3"},
        },
        "devices_state": {
        ...
        "tasks_state": {
            "t1": {"predicted_duration_min": 30, "available_time_min": 25, "expected_units": 20, "load_add": 0.2},
            "t2": {"predicted_duration_min": 20, "available_time_min": 25, "expected_units": 15, "load_add": 0.15},
            "t3": {"predicted_duration_min": 15, "available_time_min": 25, "expected_units": 10, "load_add": 0.1},
        },
        "shift": {
            "remaining_hours": 4.0,
...
class ScenarioSimulatorTest(unittest.TestCase):
    ...
    def test_capacity_vs_safety_tradeoff(self):
        ...
        self.assertGreaterEqual(cap.metrics.estimated_output, safe.metrics.estimated_output - 1e-9)
        # SAFETY_BALANCED 高负荷人员数 <= CAPACITY_FIRST（负荷均衡不超阈值）
        self.assertLessEqual(len(safe.metrics.high_load_persons), len(cap.metrics.high_load_persons))
...
class CompareTest(unittest.TestCase):
    ...
    def test_winner_by_metric_correct(self):
        ...
        plan_ids = {p.plan_id for p in plans}
        # 每个 winner 指向某个方案
        for _key, winner in comp.winner_by_metric.items():
            self.assertIn(winner, plan_ids)
        # 产量最优 = CAPACITY_FIRST（产能优先打包到快人员）
        cap_id = next(p.plan_id for p in plans if p.plan_type == PlanType.CAPACITY_FIRST.value)
        self.assertEqual(comp.winner_by_metric["estimated_output"], cap_id)
        # 高负荷人员数最优 = SAFETY_BALANCED（0 人）
        safe_id = next(p.plan_id for p in plans if p.plan_type == PlanType.SAFETY_BALANCED.value)
    ...
    def test_recommendation_references_computed_values(self):
        ...
        # 产能优先方案的高负荷人数出现在理由中
        self.assertIn(str(len(cap.metrics.high_load_persons)), comp.recommendation)
