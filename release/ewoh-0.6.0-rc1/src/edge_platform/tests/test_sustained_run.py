...
class SustainedRunNoLossTest(unittest.TestCase):
    ...
    def test_30_minute_continuous_stream_no_frame_loss(self):
        inj = WireInjector(
            device_id="EXO-SR-01", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000, battery_pct=80
        )
        ...
        self.assertLessEqual(len(adapter._seen_seqs), 4096, "_seen_seqs 应受去重窗口限制（无内存泄漏）")
        self.assertLessEqual(len(adapter._seen_seq_order), 4096)
        # _telemetry_ts_window 受采样窗口限制
        self.assertLessEqual(len(adapter._telemetry_ts_window), 1000, "_telemetry_ts_window 应受采样窗口限制")
    ...
    def test_no_packet_loss_for_contiguous_sequence(self):
        """连续 SEQ 不应有丢包率。"""
        inj = WireInjector(device_id="EXO-SR-02", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000)
...
class SustainedRunStorageTest(unittest.TestCase):
    ...
    def test_storage_records_match_frame_count(self):
        inj = WireInjector(device_id="EXO-SR-03", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000)
...
class SustainedRunInferenceTest(unittest.TestCase):
    ...
    def test_inference_continuous_throughout_stream(self):
        ...
        inj = WireInjector(
            device_id="EXO-SR-04", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000, battery_pct=80
        )
        adapter = NyExoA1Adapter("EXO-SR-04", source_type="controlled_test", worker_id="P-001")
        ...
        expected = (n - 40) // 20 + 1
        self.assertEqual(inference_count, expected, "推理结果数应与步长推算一致（无中断）")
    ...
    def test_event_state_consistent_under_continuous_stream(self):
        ...
        inj = WireInjector(
            device_id="EXO-SR-05", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000, battery_pct=80
        )
        adapter = NyExoA1Adapter("EXO-SR-05", source_type="controlled_test", worker_id="P-001")
...
class SustainedRunReconnectTest(unittest.TestCase):
    ...
    def test_disconnect_5s_reconnect_no_data_loss(self):
        inj = WireInjector(
            device_id="EXO-RC-01", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000, battery_pct=80
        )
        ...
        self.assertEqual(produced_reconnect, missed, "重连后应补传 100 帧（103 条目 - 3 重复去重）")
    ...
    def test_reconnect_after_disconnect_no_event_loss(self):
        ...
        try:
            ...
            pipeline = InferencePipeline(storage, bus, registry, rules, event_engine=event_engine)

            inj = WireInjector(
                device_id="EXO-RC-02", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000, battery_pct=80
            )
            adapter = NyExoA1Adapter("EXO-RC-02", source_type="controlled_test", worker_id="P-001")
            ...
            self.assertGreaterEqual(len(events_after), len(events_before), "重连后事件数量不应减少")
        finally:
            storage.close()
...
class SustainedRunMemoryBoundedTest(unittest.TestCase):
    ...
    def test_keep_raw_ring_bounded(self):
        ...
        inj = WireInjector(device_id="EXO-MEM-01", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000)
        adapter = NyExoA1Adapter("EXO-MEM-01", source_type="controlled_test", keep_raw=True, raw_ring_size=128)
        adapter.start()
        # 投递 1000 帧 >> raw_ring_size
        adapter.feed(inj.telemetry_burst(1000, action="walk"))
        adapter.drain()
        # 环形缓冲应不超过 raw_ring_size
        self.assertLessEqual(len(adapter._raw_ring), 128, "原始帧环形缓冲应受 raw_ring_size 限制")
        self.assertEqual(len(adapter._raw_ring), 128)

    def test_seen_seqs_bounded_under_large_stream(self):
        """36000 帧后 _seen_seqs 不超过去重窗口。"""
        inj = WireInjector(device_id="EXO-MEM-02", source_label="controlled_test", hz=20.0, start_ts_ms=1_000_000)
