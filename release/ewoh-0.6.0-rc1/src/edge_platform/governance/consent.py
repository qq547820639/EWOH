    ...
    def __post_init__(self):
        ...
        for a in self.actions:
            if a.get("action") not in (DELETE, ANONYMIZE, HANDOVER):
                raise ValueError(f"未知撤回动作: {a.get('action')!r}")
...
class ConsentManager:
    ...
    def grant(self, person_id, purposes, fields, granted_by, retention_rule=""):
        ...
        entry = self._log(
            ...
            granted_by,
            rec.record_id,
            detail=f"用途={','.join(p.value for p in purposes)}",
        )
    ...
    def _build_revocation_job(self, rec, actor_id):
            ...
            actions.append(
                {
                    ...
                    "action": action,
                    "target": f"person:{rec.person_id}/purpose:{purpose.value}",
                }
            )
            ...
            actions.append(
                {
                    "data_class": "field",
                    "action": ANONYMIZE,
                    "target": f"person:{rec.person_id}/field:{f}",
                }
            )
        job = RevocationJob(
        ...
        self._log("revocation_job", rec.person_id, actor_id, job.job_id, detail=f"actions={len(actions)}")
        return job
