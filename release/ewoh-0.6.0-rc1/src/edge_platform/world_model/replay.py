...
class Replay:
    ...
    def at(self, ts):
        ...
        events = [n.to_dict() for n in self.event_graph.all_nodes() if ts_to_ms(n.ts) <= target_ms]
    ...
    def explain_decision(self, node_id):
        ...
        return f"决策解释：{head_label} 的因果链（共 {len(chain)} 步）：{' → '.join(parts)}。"
    ...
    def _describe_node(self, node):
        ...
        t = node.node_type
        if t == "ENTER_ZONE":
            return f"员工 {p.get('person_id', '?')} 进入区域 {p.get('zone_id', '?')}"
        if t == "BIND_EXO":
            return f"与外骨骼 {p.get('device_id', '?')} 绑定"
        if t == "CLAIM_TASK":
            return f"领取任务 {p.get('task_id', '?')}"
        if t == "ARRIVE_STATION":
            return f"到达工位 {p.get('station_id', '?')}"
        if t == "START_ACTION":
            return f"开始{p.get('action', '搬运')}动作"
        if t == "LOAD_RISE":
            return f"累计负荷上升至 {p.get('load_score', p.get('value', '?'))}"
        if t == "BATTERY_DROP":
            return f"外骨骼电量下降至 {p.get('battery_pct', '?')}%"
        if t == "STATION_BACKLOG":
            return f"工位 {p.get('station_id', '?')} 出现积压（待处理 {p.get('backlog', '?')} 件）"
        if t == "SUGGESTION":
            return f"系统生成调度建议（{p.get('suggestion', '建议换人接替')}）"
        if t == "CONFIRM":
            action = p.get("action", "confirm")
            verb = "已确认" if action == "confirm" else "已驳回"
            return f"班组长{verb}（由 {p.get('confirmed_by', '班组长')} 操作）"
        if t == "REALLOCATE":
            return f"任务 {p.get('task_id', '?')} 重新分配给 {p.get('new_assignee', '?')}"
        if t == "FEEDBACK":
            return f"结果回流：{p.get('outcome', '已执行')}"
    ...
    def shift_summary(self, person_id, from_ts, to_ts):
        ...
        nodes = [
            n
            for n in self.event_graph.all_nodes()
            if from_ms <= ts_to_ms(n.ts) <= to_ms and (n.payload_json or {}).get("person_id") == person_id
        ]
        ...
        confirmed = sum(1 for n in confirms if (n.payload_json or {}).get("action", "confirm") == "confirm")
