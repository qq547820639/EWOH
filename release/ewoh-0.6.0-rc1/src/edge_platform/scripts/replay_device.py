...
--------
  ...
  python src/edge_platform/scripts/replay_device.py --session-dir recordings/sess-x \
    --disconnect-at 105 --disconnect-duration 5
...
def _connect(host, port, log):
    ...
    sock.settimeout(5.0)
    log(f"连接目标 {host}:{int(port)} ...")
    sock.connect((host, port))
    sock.settimeout(None)
    log(f"已连接 {host}:{int(port)}")
    return sock
...
def _send_frame(sock, frame_path, log):
    try:
        ...
        log(f"  读取帧文件失败 {frame_path}: {e}")
        return False
    try:
        sock.sendall(data)
    except OSError as e:
        log(f"  发送失败 (file={os.path.basename(frame_path)}): {e}")
        return False
    return True


def _replay_once(frames, session_dir, host, port, speed, shuffle, disconnect_at, disconnect_duration, source_type, log):
    ...
    try:
        ...
        for idx, frame in enumerate(order):
            ...
            if disconnect_at is not None and frame.get("seq") == disconnect_at:
                log(f"  --disconnect-at 命中 seq={disconnect_at}：断开连接")
                try:
                    sock.close()
                except OSError:
                    pass
                sock = None
                log(f"  等待 {disconnect_duration:.2f}s 后重连 ...")
...
def _parse_args(argv):
    p = argparse.ArgumentParser(description="NXP1 v1.0 设备模拟回放器：按原始时间间隔向适配层重放录制帧。")
    p.add_argument(
        "--session-dir", required=True, help="录制会话目录（含 manifest.json + index.jsonl，或手动构造的帧目录）"
    )
    ...
    p.add_argument(
        "--source-type", default="real", help="标记回放数据来源（默认 real）；仅用于演示切换，不改变发送字节"
    )
    p.add_argument("--disconnect-at", type=int, default=None, help="在发送指定 seq 的帧后断开（断线恢复测试）")
    p.add_argument("--disconnect-duration", type=float, default=3.0, help="断连后等待秒数再重连（默认 3.0）")
...
def main(argv=None):
    ...
    if not os.path.isdir(args.session_dir):
        print(f"错误：session-dir 不存在: {args.session_dir}", file=sys.stderr)
        return 2
    if args.speed <= 0:
    ...
    if not frames:
        print(f"错误：未在 {args.session_dir} 找到任何帧（无 index.jsonl 且无 *.bin）", file=sys.stderr)
        return 2

    manifest = meta.get("manifest") or {}
    log(f"回放会话目录: {os.path.abspath(args.session_dir)}")
    log(f"帧来源: {meta['source']}  帧数: {len(frames)}")
    log(
        "session_id={}  protocol={}  录制设备={}".format(
            manifest.get("session_id", "?"), manifest.get("protocol_version", "?"), manifest.get("device_ids", "?")
        )
    )
    log(
        f"目标: {args.target_host}:{args.target_port}  speed={args.speed:.2f}  "
        f"loop={args.loop}  shuffle={args.shuffle}  source_type={args.source_type}（仅演示标记）"
    )
    if args.disconnect_at is not None:
        log(f"断连注入: 在 seq={args.disconnect_at} 后断开 {args.disconnect_duration:.2f}s 再重连")
    ...
    try:
        while True:
            ...
            ok = _replay_once(
                ...
                args.shuffle,
                args.disconnect_at,
                args.disconnect_duration,
                args.source_type,
                log,
            )
            if not ok:
                log(f"回放异常退出（第 {int(loop_count)} 轮）")
                return 1
            if not args.loop:
                break
        ...
        log("收到中断，停止回放")
    log(f"回放结束，共完成 {int(loop_count)} 轮")
    return 0
