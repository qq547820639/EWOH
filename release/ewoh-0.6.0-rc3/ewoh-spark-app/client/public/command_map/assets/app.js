...
(function () {
  ...
  CM.SAMPLE_DATA = {
    ...
    entities: [
      ...
      // ---- L1 2.5D：摄像头（视锥覆盖工位） ----
      { entity_id: 'CAM-01', entity_type: 'camera', parent_id: 'WS-01', name: '摄像头-装配区1',
        pose: { x: 145, y: 120, yaw: 180 }, bbox: { w: 12, h: 12 }, status: 'online',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 2,
        camera: { fov_deg: 75, range_m: 12, height_m: 3.5, floor: 1, quality: 'good' } },
      { entity_id: 'CAM-02', entity_type: 'camera', parent_id: 'WS-01', name: '摄像头-装配区2',
        pose: { x: 385, y: 120, yaw: 180 }, bbox: { w: 12, h: 12 }, status: 'online',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 2,
        camera: { fov_deg: 75, range_m: 12, height_m: 3.5, floor: 1, quality: 'good' } },
      { entity_id: 'CAM-03', entity_type: 'camera', parent_id: 'WS-01', name: '摄像头-物流区1',
        pose: { x: 630, y: 120, yaw: 180 }, bbox: { w: 12, h: 12 }, status: 'online',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 2,
        camera: { fov_deg: 75, range_m: 12, height_m: 3.5, floor: 1, quality: 'good' } },
      { entity_id: 'CAM-04', entity_type: 'camera', parent_id: 'WS-01', name: '摄像头-物流区2',
        pose: { x: 810, y: 120, yaw: 180 }, bbox: { w: 12, h: 12 }, status: 'offline',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 2,
        camera: { fov_deg: 75, range_m: 12, height_m: 3.5, floor: 1, quality: 'bad' } },

      // ---- L1 2.5D：UWB 基站（覆盖圆） ----
      { entity_id: 'UWB-BS-01', entity_type: 'uwb_station', parent_id: 'WS-01', name: 'UWB基站-装配区',
        pose: { x: 290, y: 200, yaw: 0 }, bbox: { w: 10, h: 10 }, status: 'online',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 1,
        uwb: { coverage_r: 180, height_m: 4.0, floor: 1, accuracy_m: 0.3 } },
      { entity_id: 'UWB-BS-02', entity_type: 'uwb_station', parent_id: 'WS-01', name: 'UWB基站-物流区',
        pose: { x: 730, y: 200, yaw: 0 }, bbox: { w: 10, h: 10 }, status: 'online',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 1,
        uwb: { coverage_r: 180, height_m: 4.0, floor: 1, accuracy_m: 0.3 } },
      { entity_id: 'UWB-BS-03', entity_type: 'uwb_station', parent_id: 'WS-01', name: 'UWB基站-中央通道',
        pose: { x: 500, y: 350, yaw: 0 }, bbox: { w: 10, h: 10 }, status: 'online',
        source_type: 'real', confidence: 1.0, updated_at: '2026-07-31T08:00:00', version: 1,
        uwb: { coverage_r: 150, height_m: 4.0, floor: 1, accuracy_m: 0.3 } }
    ],
    ...
    function applyBackendData(payload) {
      ...
      CM.SAMPLE_DATA.entities.forEach(function (e) {
        if (e.entity_type === 'workshop' || e.entity_type === 'zone' ||
            ...
            e.entity_type === 'camera' || e.entity_type === 'uwb_station') {
          newEntities.push(e);
        }
      });
  ...
  CM.setLevel = function (level) {
    CM.state.mapLevel = level;
    document.querySelectorAll('.cm-level-btn').forEach(function (b) {
      b.classList.toggle('active', b.dataset.level === level);
    });
    var meta = document.getElementById('map-meta');
    var levelLabel = level === 'L2' ? 'L2 轻量三维（待GLB资产）' : level === 'L1' ? 'L1 2.5D 地图' : 'L0 二维地图';
    if (meta) {
      meta.textContent = '来源 ' + (CM.state.dataSource === 'backend' ? 'LIVE 后端' : '离线样本') +
        ' · 坐标系 工厂局部（米）· ' + levelLabel;
    }
    // L2: 隐藏SVG地图，显示3D占位
    var svgEl = document.getElementById('map-svg');
    var wrap = document.querySelector('.cm-map-wrap');
    var placeholder = document.getElementById('cm-l2-placeholder');
    if (level === 'L2') {
      if (svgEl) svgEl.style.display = 'none';
      if (!placeholder && wrap) {
        placeholder = document.createElement('div');
        placeholder.id = 'cm-l2-placeholder';
        placeholder.className = 'cm-l2-placeholder';
        placeholder.innerHTML =
          '<div class="cm-l2-icon">📊</div>' +
          '<div class="cm-l2-title">L2 轻量三维模型（GLB/glTF）</div>' +
          '<div class="cm-l2-desc">当前为占位骨架。真实三维资产需由 CAD 或现场扫描生成 GLB/glTF 文件，' +
          '通过空间资产注册表（spatial/asset_registry.py）导入后在此加载。</div>' +
          '<div class="cm-l2-note">L3 高写实局部场景（点云/Gaussian Splat）仅用于重点工位事故复盘与培训，按需启用。</div>' +
          '<div class="cm-l2-spec">验收目标：重点工位误差≤10cm · 场景加载≤5秒 · 普通办公终端流畅</div>';
  ...
  CM.rerenderAll = function () {
    ...
    if (CM.state.activeTab === 'workbench' && CM.workbench) CM.workbench.render();
  ...
  CM.init = function () {
    ...
    CM.initAssistant();
    document.querySelectorAll('.cm-level-btn').forEach(function (b) {
      b.addEventListener('click', function () { CM.setLevel(b.dataset.level); });
    });
    CM.renderTopbar();
    CM.layers.render();
    CM.map.render();
    ...
    CM.admin.render();
    if (CM.workbench) CM.workbench.render();
    CM.tick();
    setInterval(CM.tick, 1000);

    // 尝试连接 backend；失败回退 sample data
    // iframe 内（妙搭应用嵌入）：直接用 sample data，不连本地 backend
    if (window.self !== window.top) {
      console.log('[CM] iframe 环境，使用 sample data');
