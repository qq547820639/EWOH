...
Raw Kubernetes manifests are in `deploy/cloud/k8s/`. For repeatable releases,
use the Helm chart:
...
helm template ewoh deploy/cloud/helm/ewoh \
  ...
  --set image.tag=0.6.0-rc3 \
  --set migration.image.repository=registry.example/ewoh-migrate \
  --set factory.id=factory-001 \
  --set factory.name="Factory 001" \
  --set factory.upgradeRing=pilot \
  --set ingress.host=ewoh.example.com
```

The chart renders namespace, ConfigMap, migration Job hook, Deployment,
Service, Ingress, HPA, PDB, and local-storage PVC. Runtime credentials are
never generated by the chart; create `ewoh-api-secret` and
`ewoh-migration-secret` in the cluster first. `scripts/verify-helm-chart.js`
validates the chart structure and all `.Values.*` references without needing a
cluster.
