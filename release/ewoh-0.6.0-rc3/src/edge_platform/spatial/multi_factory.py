...
class MultiFactoryRegistry:
    ...
    def register_factory(self, factory_node):
        ...
        self._audit(
            "register_factory",
            factory_id=factory_node.factory_id,
    ...
    def register_link(self, link):
        ...
        self._audit(
            ...
            source=link.source_factory_id,
            target=link.target_factory_id,
    ...
    def list_links(self, factory_id=None):
        ...
        return [
            link
            for link in self._links.values()
            if link.source_factory_id == factory_id or link.target_factory_id == factory_id
        ]
    ...
    def validate_federation(self, factory_id_a, factory_id_b, data_class):
        ...
        if fa is None or fb is None:
            reason = f"工厂不存在: {factory_id_a} / {factory_id_b}"
            self._audit(
                "validate_federation",
                factory_a=factory_id_a,
                factory_b=factory_id_b,
        ...
        if link is None:
            reason = f"无 ACTIVE 跨工厂链接: {factory_id_a} <-> {factory_id_b}"
            self._audit(
                "validate_federation",
                factory_a=factory_id_a,
                factory_b=factory_id_b,
        ...
        if policy is None:
            reason = f"无联邦策略允许共享数据类别: {data_class}"
            self._audit(
                "validate_federation",
                factory_a=factory_id_a,
                factory_b=factory_id_b,
...
class CrossFactorySchedulerStub:
    ...
    def propose_cross_factory(self, task, candidate_factories, ctx=None):
        ...
        for factory_id in candidate_factories:
            ...
            try:
                ...
                self._audit("propose_cross_factory_local_error", factory_id=factory_id, error=str(exc))
                continue
    ...
    def validate_isolation(self, plan):
        ...
        ok = len(violations) == 0
        self._audit(
            "validate_isolation", factories=factories, data_classes=data_classes, ok=ok, violation_count=len(violations)
        )
