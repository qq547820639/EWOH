...
class ActionModel:
    ...
    def fit(self, samples):
        ...
        rows = [(features, label) for features, label in rows if isinstance(features, dict) and label]
        if not rows:
        ...
        for features, _ in rows:
            try:
                raw.append([float(features[k]) for k in self.feature_names])
            except (KeyError, TypeError, ValueError) as e:
                raise ValueError(f"训练样本特征缺失/非法: {e}") from e
        m = len(raw)
        ...
        for (_, label), r in zip(rows, raw):
            ...
            for i in range(n):
                sums[label][i] += z[i]
            cnt[label] += 1
        self.centroids = {label: [s / cnt[label] for s in sums[label]] for label in sums}
    ...
    def from_dict(cls, d):
        try:
            ...
            m.centroids = {str(k): [float(v) for v in vs] for k, vs in d["centroids"].items()}
            m.mean = [float(x) for x in d["stats"]["mean"]]
            m.std = [max(float(x), 1e-9) for x in d["stats"]["std"]]
            m.thresholds = {
                "min_confidence": float(d["thresholds"]["min_confidence"]),
                "ambiguous_gap": float(d["thresholds"]["ambiguous_gap"]),
            }
            m.trained_at = d.get("trained_at")
            m.dataset_version = d.get("dataset_version")
        except (KeyError, TypeError, ValueError) as e:
            raise ModelError(f"模型文件字段缺失/非法: {e}") from e
        n = len(m.feature_names)
        ok = (
            m.centroids
            and len(m.mean) == n
            and len(m.std) == n
            and all(len(c) == n and all(math.isfinite(v) for v in c) for c in m.centroids.values())
        )
        if not ok:
    ...
    def load(cls, path):
        ...
        try:
            ...
            raise ModelError(f"模型文件不可读: {e}") from e
...
class ModelRegistry:
    ...
    def activate(self, version):
        ...
        if version not in reg["models"]:
            raise ModelError(f"版本未注册: {version}")
