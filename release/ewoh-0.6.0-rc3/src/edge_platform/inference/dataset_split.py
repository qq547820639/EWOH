...
def _person_ids(samples):
    ...
    for i, s in enumerate(samples):
        ...
        if pid is None or pid == "":
            raise ValueError(f"第 {int(i)} 个样本缺少 person_id")
        ids.add(pid)
    return ids
...
def split_by_person(samples, ratios=(0.7, 0.15, 0.15), seed=42):
    ...
    if abs(sum(ratios) - 1.0) > 1e-9:
        raise ValueError(f"ratios 之和必须为 1.0，实际为 {sum(ratios)!r}")
    if any(r < 0 for r in ratios):
        raise ValueError(f"ratios 各分量不得为负: {ratios!r}")
...
def verify_no_person_leak(splits):
    ...
    pid_sets = {
        k: {s.get("person_id") for s in splits[k] if isinstance(s, dict) and s.get("person_id") is not None}
        for k in keys
    }
    overlap = {}
    ok = True
...
def write_split_manifest(splits, path, ratios=None):
    ...
    person_ids = {k: sorted({s["person_id"] for s in splits.get(k, [])}) for k in _SPLIT_KEYS}
    if ratios is None:
