...
class PurgeExecutor:
    ...
    def _audit(self, storage, action, target_type, target_id, detail, before=None, after=None):
        """记录一条清理审计日志。"""
        ...
        if storage is not None and hasattr(storage, "insert_audit_log"):
            return storage.insert_audit_log(
                action, "system", target_type, target_id, before=before, after=after, result="success"
            )
        return None
    ...
    def _batched_delete(storage, table, condition, params, batch_size):
        ...
        """
        deleted = 0
        subsql = f"SELECT rowid FROM {table} WHERE {condition} LIMIT ?"  # nosec B608 - internal table/condition mapping
        delsql = f"DELETE FROM {table} WHERE rowid IN ({subsql})"  # nosec B608 - internal table/condition mapping
        while True:
    ...
    def dry_run(self, storage, data_class) -> int:
        ...
        sql = f"SELECT COUNT(*) c FROM {table} WHERE {condition}"  # nosec B608 - internal table/condition mapping
