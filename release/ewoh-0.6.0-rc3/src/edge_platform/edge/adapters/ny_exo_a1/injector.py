...
class WireInjector:
    ...
    def __init__(
        self,
        device_id="EXO-CT-001",
        source_label="controlled_test",
    ...
    def heartbeat(self, battery_pct=None, status=0):
        ...
        return self._emit(
            codec.encode_heartbeat(
                self.battery_pct, status=status, seq=seq, ts_ms=ts, firmware_version=self.firmware_version
            )
        )
    ...
    def telemetry(self, action="stand", **overrides):
        """TELEMETRY(0x10)：单帧遥测。"""
        seq, ts = self._advance()
        return self._emit(codec.encode_telemetry(seq=seq, ts_ms=ts, **self._telemetry_fields(action, **overrides)))
    ...
    def disconnect(self, missed_frames=10, action="lift"):
        ...
        for _ in range(int(missed_frames)):
            seq, ts = self._advance()
            self._cache.append({"seq": seq, "ts_ms": ts, "telemetry": self._telemetry_fields(action)})
    ...
    def timestamp_backwards(self, frame_bytes=None, action="stand"):
        ...
        if frame_bytes is None:
            ...
            return self._emit(
                self._rewrite_ts(
                    codec.encode_telemetry(seq=seq, ts_ms=ts, **self._telemetry_fields(action)), backward_ts
                )
            )
    ...
    def timestamp_drift(self, frame_bytes=None, drift_ms=1000, action="stand"):
        ...
        if frame_bytes is None:
            ...
            return self._emit(
                self._rewrite_ts(
                    codec.encode_telemetry(seq=seq, ts_ms=ts, **self._telemetry_fields(action)), drifted_ts
                )
            )
        frame, _ = protocol.decode_frame(frame_bytes)
        if frame is None:
    ...
    def scenario(self, name):
        ...
        if name not in SCENARIOS:
            raise ValueError(f"未知场景: {name}（可用: {', '.join(SCENARIOS)}）")
        if name == "normal":
        ...
        if name == "crc_error":
            return self.ident() + self.telemetry_burst(2, "walk") + self.corrupted() + self.telemetry_burst(2, "walk")
        # degraded
        return self.ident() + self.missing_field() + self.out_of_range() + self.low_battery()
