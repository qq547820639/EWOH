...
class ConstraintViolation:
    ...
    def __post_init__(self):
        if self.constraint_type not in VALID_CONSTRAINT_TYPES:
            raise ValueError(f"未知硬约束类型: {self.constraint_type!r}")
    ...
    def __str__(self):
        return f"[{self.constraint_type}] {self.reason}"
...
class HardConstraints:
    ...
    def _check_skill(self, person, task):
        ...
        if missing:
            return ConstraintViolation(
                SKILL,
                pid,
                f"人员 {pid} 缺少任务所需技能：{'、'.join(sorted(missing))}",
            )
        return None
    ...
    def _check_station_auth(self, person, task):
        ...
        if station_id and station_id not in authorized:
            return ConstraintViolation(
                STATION_AUTH,
                pid,
                f"人员 {pid} 未取得工位 {station_id} 的作业授权",
            )
        return None
    ...
    def _check_health(self, person, task):
        ...
        if load_level and load_level in forbidden:
            hits.append(f"负荷等级 {load_level}")
        action_type = task.get("action_type")
        if action_type and action_type in forbidden:
            hits.append(f"动作类型 {action_type}")
        if hits:
            return ConstraintViolation(
                HEALTH_TABOO,
                pid,
                f"人员 {pid} 健康禁忌命中：{'、'.join(hits)}",
            )
        return None
    ...
    def _check_shift_rest(self, person, task, ctx):
        ...
        if max_continuous and continuous > max_continuous:
            return ConstraintViolation(
                SHIFT_REST,
                pid,
                f"人员 {pid} 连续作业 {continuous:.0f} 分钟超过班次上限 {max_continuous:.0f} 分钟，需休息",
            )
        ...
        if rest_per_hour and worked_hour > (60.0 - rest_per_hour) + 1e-9:
            return ConstraintViolation(
                SHIFT_REST,
                pid,
                f"人员 {pid} 本小时已工作 {worked_hour:.0f} 分钟，未满足每小时休息 {int(rest_per_hour)} 分钟的规则",
            )
        return None
    ...
    def _check_exo_compat(self, person, task, device):
        ...
        if missing:
            return ConstraintViolation(
                EXO_MODEL_COMPAT,
                pid,
                f"外骨骼型号 {model} 不满足任务要求：{'、'.join(sorted(missing))}",
            )
        return None
