...
WITH expected(name) AS (VALUES ('ewoh_asset_package'), ('ewoh_audit_log'), ('ewoh_control_command'), ('ewoh_control_request'), ('ewoh_control_result'), ('ewoh_device_binding'), ('ewoh_device_capability'), ('ewoh_event_action'), ('ewoh_event_rule'), ('ewoh_event_subscription'), ('ewoh_factory_profile'), ('ewoh_factory_template'), ('ewoh_knowledge_base'), ('ewoh_knowledge_entry'), ('ewoh_model_asset'), ('ewoh_model_binding'), ('ewoh_notification'), ('ewoh_person_role'), ('ewoh_person_skill'), ('ewoh_resource_binding'), ('ewoh_resource_preorder'), ('ewoh_role'), ('ewoh_schedule_assignment'), ('ewoh_schedule_task'), ('ewoh_schedule_task_step'), ('ewoh_skill'), ('ewoh_spatial_hierarchy'), ('ewoh_spatial_relation'), ('ewoh_system_config'), ('ewoh_task_skill_req'), ('ewoh_task_step'), ('ewoh_task_template'), ('ewoh_workstation'), ('ewoh_workstation_device'), ('ewoh_workstation_person'), ('ewoh_workstation_relation'), ('ewoh_workstation_skill'), ('ewoh_world_delta_log'), ('ewoh_world_snapshot'), ('ewoh_scheduler_config'), ('ewoh_environment'), ('ewoh_model_registry'), ('ewoh_schedule_audit'), ('ewoh_schedule_plan'), ('ewoh_event_chain'), ('ewoh_world_state'), ('ewoh_topology'), ('ewoh_spatial_entity'), ('ewoh_telemetry'), ('ewoh_event'), ('ewoh_device')),
...
managed AS (
  ...
  FROM expected e
  JOIN information_schema.tables t ON t.table_schema = '__EWOH_SCHEMA__' AND t.table_name = e.name
),
missing_org AS (
  SELECT count(*) AS missing_org_id
  FROM expected e
  LEFT JOIN information_schema.columns c ON c.table_schema = '__EWOH_SCHEMA__' AND c.table_name = e.name AND c.column_name = 'org_id'
  WHERE c.column_name IS NULL
),
nullable_org AS (
  SELECT count(*) AS org_not_null_violations
  FROM information_schema.columns c
  WHERE c.table_schema = '__EWOH_SCHEMA__' AND c.table_name = ANY (ARRAY['ewoh_asset_package', 'ewoh_control_command', 'ewoh_control_request', 'ewoh_control_result', 'ewoh_device_binding', 'ewoh_device_capability', 'ewoh_event_action', 'ewoh_event_rule', 'ewoh_event_subscription', 'ewoh_factory_profile', 'ewoh_factory_template', 'ewoh_knowledge_base', 'ewoh_knowledge_entry', 'ewoh_model_asset', 'ewoh_model_binding', 'ewoh_notification', 'ewoh_person_role', 'ewoh_person_skill', 'ewoh_resource_binding', 'ewoh_resource_preorder', 'ewoh_role', 'ewoh_schedule_assignment', 'ewoh_schedule_task', 'ewoh_schedule_task_step', 'ewoh_skill', 'ewoh_spatial_hierarchy', 'ewoh_spatial_relation', 'ewoh_task_skill_req', 'ewoh_task_step', 'ewoh_task_template', 'ewoh_workstation', 'ewoh_workstation_device', 'ewoh_workstation_person', 'ewoh_workstation_relation', 'ewoh_workstation_skill', 'ewoh_scheduler_config', 'ewoh_environment', 'ewoh_model_registry', 'ewoh_schedule_audit', 'ewoh_schedule_plan', 'ewoh_event_chain', 'ewoh_world_state', 'ewoh_topology', 'ewoh_spatial_entity', 'ewoh_telemetry', 'ewoh_event', 'ewoh_device']) AND c.column_name = 'org_id' AND c.is_nullable = 'YES'
),
...
rls AS (
  ...
  JOIN pg_namespace n ON n.oid = c.relnamespace AND n.nspname = '__EWOH_SCHEMA__'
...
policy_missing AS (
  ...
  FROM (
    SELECT e.name
    FROM expected e
    LEFT JOIN pg_policies p ON p.schemaname = '__EWOH_SCHEMA__' AND p.tablename = e.name
    GROUP BY e.name
...
loose AS (
  ...
  WHERE p.schemaname = '__EWOH_SCHEMA__' AND (p.qual = 'true' OR p.with_check = 'true')
),
auth_dml AS (
...
identities AS (
  SELECT
    count(*) FILTER (WHERE a.attrelid = to_regclass('__EWOH_SCHEMA__.ewoh_audit_log') AND a.attname = 'audit_seq' AND a.attidentity = 'a') AS audit_seq_identity,
    count(*) FILTER (WHERE a.attrelid = to_regclass('__EWOH_SCHEMA__.ewoh_world_delta_log') AND a.attname = 'seq' AND a.attidentity = 'a') AS world_delta_seq_identity
  FROM pg_attribute a
),
audit_fn AS (
  SELECT count(*) AS audit_function_count
  FROM pg_proc p
  JOIN pg_namespace n ON n.oid = p.pronamespace AND n.nspname = '__EWOH_SCHEMA__'
...
config_unique AS (
  ...
  FROM pg_indexes
  WHERE schemaname = '__EWOH_SCHEMA__' AND tablename = 'ewoh_scheduler_config' AND indexname = 'uq_ewoh_scheduler_config_org_key'
) SELECT
