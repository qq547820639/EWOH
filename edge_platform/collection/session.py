...
SESSION_FIELDS = (
    "person_id", "device_id", "firmware_version",
    "wear_position", "wear_tightness", "action_code",
    "repetitions", "load_level", "environment", "ground",
    "consent_id", "observer", "notes", "subjective_feedback", "exception",
)
...
class SessionManager:
    ...
    def start_session(self, **fields):
        ...
        if unknown:
            raise ValueError("未知会话字段: %s" % sorted(unknown))
        sid = new_id("SES")
        ...
        with closing(self._conn()) as c:
            c.execute("INSERT INTO collection_session (%s) VALUES (%s)"
    ...
    def stop_session(self, session_id):
        with closing(self._conn()) as c:
            ...
            c.commit()
        if cur.rowcount == 0:
            raise KeyError("会话不存在: %s" % session_id)

    def get_session(self, session_id):
        with closing(self._conn()) as c:
            row = c.execute("SELECT * FROM collection_session WHERE session_id=?",
                            (session_id,)).fetchone()
            if row is None:
                return None
            sess = dict(row)
            sess["labels"] = [dict(r) for r in c.execute(
                "SELECT * FROM collection_label WHERE session_id=? ORDER BY start_ts",
                (session_id,))]
    ...
    def list_sessions(self):
        with closing(self._conn()) as c:
            return [dict(r) for r in c.execute(
                "SELECT * FROM collection_session ORDER BY start_ts")]

    # ---- 标签 ----
    def add_label(self, session_id, action_code, start_ts, end_ts,
                  labeler, quality, aux_tags):
        """给会话添加人工动作标签（起止时间为 ISO 字符串），返回 label_id。"""
        lid = new_id("LBL")
        with closing(self._conn()) as c:
            if c.execute("SELECT 1 FROM collection_session WHERE session_id=?",
            ...
            c.execute(
                ...
                " VALUES (?,?,?,?,?,?,?,?)",
                (lid, session_id, action_code, start_ts, end_ts,
                 labeler, quality, json.dumps(list(aux_tags or []), ensure_ascii=False)))
            c.commit()
        return lid
