...
def person_metrics(storage, person, device):
    ...
    open_events = [e for e in (norm_event(x) for x in storage.list_events(200))
    ...
    return {"current_load": round(load, 3), "work_minutes": round(work_min, 1),
            "open_events": len(open_events), "open_high_events": len(recent_high), "risk_recent": round(risk, 3)}
...
def recommend(storage, assignments, payload, device_online):
    ...
    for p in storage.list_people():
        ...
        parts = {"技能匹配": round(WEIGHTS["技能匹配"] * skill_ok, 1),
        ...
        blocked = []
        if not skill_ok:
            blocked.append("无技能：不具备任务所需技能「%s」" % skill)
        if m["open_high_events"]:
            blocked.append("高风险未解除：存在 %d 条未处置高风险事件" % m["open_high_events"])
        if not dev:
            blocked.append("设备故障：未绑定可用外骨骼设备")
        elif not device_online(dev):
            blocked.append("设备故障：设备 %s 离线或故障" % dev.get("device_id"))
        if p.get("consent_status") != "granted":
            blocked.append("未授权：人员授权状态为 %s" % p.get("consent_status", "unknown"))
        if not p.get("active", 1):
            blocked.append("不在岗：人员状态为停用")
        elif p.get("person_id") in assigned:
            blocked.append("任务冲突：已被确认分配其他任务")
        rows.append({"person_id": p.get("person_id"), "name": p.get("display_name", p.get("name", "")),
                     "eligible": not blocked, "blocked_by": blocked,
                     "score": round(sum(parts.values()), 1), "reasons": parts, "metrics": m})
    rows.sort(key=lambda x: (x["eligible"], x["score"]), reverse=True)
    return {"weights": WEIGHTS, "requires_human_confirm": True,
...
def confirm_assignment(storage, assignments, payload, device_online):
    ...
    task_id = payload.get("task_id") or "TASK-%s" % datetime.now().strftime("%H%M%S")
    person_id = payload.get("person_id")
    confirmer = (payload.get("confirmer") or "").strip()
    if not confirmer:
        return {"ok": False, "error": "必须填写确认人（班组长或现场负责人），禁止自动派工。"}
    cand = [r for r in recommend(storage, assignments, payload, device_online)["items"]
    ...
    rec = {"task_id": task_id, "person_id": person_id, "confirmer": confirmer,
           "status": "confirmed", "confirmed_at": iso(datetime.now())}
...
REFUSE_RULES = [
    ("医学诊断", ["诊断", "疾病", "病历", "医疗", "受伤", "健康评估", "体检"],
     "平台不提供医学诊断。本平台仅用于作业安全协同，授权用途不包括任何健康或医学判断。"),
    ("惩罚性人事", ["开除", "降薪", "惩罚", "处罚", "扣钱", "裁员", "保险定价", "考核扣分"],
     "平台不支持惩罚性绩效、保险定价等用途，该用途已被安全边界明确禁止。"),
    ("修改安全阈值", ["修改阈值", "调高阈值", "放宽阈值", "改阈值", "安全阈值", "限扭"],
     "急停、限扭、关节实时控制等安全闭环参数由设备本地控制，平台与大模型无权写入或修改。"),
    ("控制关节/急停", ["控制关节", "急停", "远程关机", "停下设备", "停止外骨骼", "远程控制"],
     "平台不能控制关节、急停或任何设备实时控制回路，该能力在架构上不可绕过地保留在设备本地。"),
    ("未授权人体数据查询", ["隐私", "原始人体数据", "个人数据", "体征", "生理"],
     "查询未授权人体数据超出授权边界。仅可查询已签署授权的匿名化作业记录。"),
    ("包装受控结果", ["包装", "冒充", "当成客户", "客户现场结论", "伪造", "说成真机"],
...
def _no_data():
    return {"answer": "本地持久层中没有可引用的记录，按白名单策略拒答，不提供无数据依据的结论。",
...
def answer(storage, question, device_online, assignments, source=None):
    ...
    if "在线" in q or ("多少" in q and "设备" in q):
        views = [{"device_id": d["device_id"], "online": device_online(d),
                  "source_type": d.get("source_type"), "last_seen": d.get("last_seen")} for d in devices]
        on = [v for v in views if v["online"]]
        return {"answer": "当前在线设备 %d 台，离线 %d 台：%s。" % (
                    len(on), len(views) - len(on),
                    "、".join("%s(%s)" % (v["device_id"], v["source_type"]) for v in on) or "无"),
                "evidence": views, "refused": False, "category": "在线设备"}
    # 6. 设备掉线情况
    if "掉线" in q or "离线" in q or "断连" in q:
        off = [{"device_id": d["device_id"], "last_seen": d.get("last_seen"), "source_type": d.get("source_type")}
               for d in devices if not device_online(d)]
        return {"answer": "当前离线设备 %d 台：%s。恢复通信后平台无需重启即可自动恢复显示。" % (
    ...
    if "负荷" in q:
        ...
        return {"answer": "最近时间窗内最高负荷分数 %.2f，来自设备 %s（来源 %s），时间 %s。" % (
                    top["telemetry"].get("load_score", 0), top.get("device_id"),
                    top.get("source_type"), top.get("timestamp")),
                "evidence": [{"record_id": top.get("record_id"), "timestamp": top.get("timestamp"),
                              "device_id": top.get("device_id"), "source_type": top.get("source_type")}],
                "refused": False, "category": "最高负荷"}
    # 3/4. 近期风险事件与告警原因
    if "事件" in q or "风险" in q or "告警" in q or "报警" in q:
        ...
        if "原因" in q or "为什么" in q:
            e = events[0]
            trig = e.get("trigger", {})
            return {"answer": "最近一条事件 %s（等级 %s）触发依据：%s（规则/模型版本 %s）。" % (
                        e.get("event_code"), e.get("severity"),
                        trig.get("condition", trig.get("type", "见触发详情")),
                        trig.get("rule_version") or trig.get("model_version", "未知")),
                    "evidence": [{"event_id": e.get("event_id"), "event_code": e.get("event_code"),
                                  "trigger": trig, "source_type": e.get("source_type")}],
                    "refused": False, "category": "告警原因"}
        return {"answer": "最近共有 %d 条风险事件，最近一条为 %s（%s，等级 %s，状态 %s）。" % (
                    len(events), events[0].get("event_code"), events[0].get("start_time"),
                    events[0].get("severity"), events[0].get("status")),
                "evidence": [{"event_id": e.get("event_id"), "event_code": e.get("event_code"),
                              "severity": e.get("severity"), "status": e.get("status"),
                              "start_time": e.get("start_time"), "source_type": e.get("source_type")}
                             for e in events[:5]],
                "refused": False, "category": "近期风险事件"}
    # 5. 任务适配人选
    if "谁适合" in q or "派谁" in q or "人选" in q or "推荐" in q:
        ...
        if not top:
            return {"answer": "当前没有满足全部硬约束的可派工人员（技能：%s）。" % skill,
                    "evidence": res["items"], "refused": True, "category": "任务适配人选"}
        return {"answer": "按 30/20/20/15/15 可解释评分，技能「%s」首选 %s（得分 %.1f）。"
                          "最终派工须班组长人工确认，平台不自动派工。" % (skill, top["name"], top["score"]),
                "evidence": [top], "refused": False, "category": "任务适配人选"}
    # 7. 某时段动作记录
    if "动作" in q:
        now = datetime.now().astimezone()
        start = iso(now - timedelta(minutes=_window(q)))
        dev_id = next((d["device_id"] for d in devices if d["device_id"] in q), devices[0]["device_id"] if devices else None)
        if not dev_id:
            return _no_data()
        ...
        desc = "、".join("%s %d 次" % kv for kv in sorted(labels.items(), key=lambda x: -x[1]))
        return {"answer": "设备 %s 在该时段的动作记录：%s（模型版本 %s）。" % (
                    dev_id, desc, inf[-1].get("model_version", "未知")),
                "evidence": [{"inference_id": r.get("inference_id"), "label": r.get("label"),
                              "confidence": r.get("confidence"), "ts_end": r.get("ts_end"),
                              "model_version": r.get("model_version"), "source_type": r.get("source_type")}
                             for r in inf[-10:]],
                "refused": False, "category": "某时段动作记录"}
    # 8. 数据来源
    if "来源" in q or "真实" in q or "模拟" in q:
        by_src = {}
        for d in devices:
            by_src.setdefault(d.get("source_type"), []).append(d["device_id"])
        desc = "；".join("%s：%s" % (k, "、".join(v)) for k, v in by_src.items()) or "无设备"
        return {"answer": "平台数据来源分三类：real（真实设备）、controlled_test（受控采集）、simulated（模拟，仅工程自测）。"
                          "当前接入：%s。所有页面与导出均携带来源标识。" % desc,
                "evidence": [{"source_type": k, "devices": v} for k, v in by_src.items()],
                "refused": False, "category": "数据来源"}
    return {"answer": "该问题不在白名单内。我可回答：在线设备、最高负荷、近期风险事件、告警原因、"
...
def evaluate_scenario(p):
    ...
    def num(k, d):
        try:
            ...
            return d
    parts = {"空间结构化": num("structured", 3) * 4, "ROI可量化": num("roi", 3) * 4,
             "付费方明确": num("payer", 3) * 3, "设备适配": num("fit", 3) * 3,
             "数据合规": num("compliance", 3) * 3, "复制价值": num("replicate", 3) * 3}
    ...
    conclusion = ("不推荐：命中一票否决（%s）" % "、".join(vetoes)) if vetoes else \
                 ("推荐试点" if rating == "高" else "有条件推荐：补齐短板后试点" if rating == "中" else "暂不推荐")
    one_pager = {
        "场景": p.get("scene", "候选场景"), "工序": p.get("process", "搬运/装卸工序"),
        "人数": people, "动作": p.get("actions", "站立/行走/弯腰/搬举"),
        "痛点": p.get("pain", "高负荷搬运导致疲劳与工伤风险"),
        "付费方": p.get("payer_name", "待确认"), "接口": p.get("interfaces", "AIoT/门禁/定位接口待比对"),
        "数据条件": "需签署授权与 DPA，基线数据可采集",
        "设备适配度": "%d/5" % int(num("fit", 3)),
        "试点周期": "%d 周" % (8 if rating == "高" else 10),
        "建议KPI": ["特定工序作业时间", "单位人力有效产出", "负荷/疲劳趋势", "设备连续运行与故障率"],
        "风险": vetoes or ["人员接受度", "现场网络条件", "基线数据质量"],
        "推荐结论": conclusion}
    return {"score": score, "rating": rating, "vetoed": bool(vetoes), "veto_reasons": vetoes,
            "recommended": recommended, "score_parts": {k: round(v, 1) for k, v in parts.items()},
            "recommended_devices": dev_n, "backup_devices": backup, "one_pager": one_pager,
            "next_requests": ["捷顺指定战略/技术/销售方案/项目交付四条对接线",
