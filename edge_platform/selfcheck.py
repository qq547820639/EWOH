...
def main():
    ...
    storage.insert_event({
        "event_id": "EVT-CHECK0001", "event_code": "LOAD_CONTINUOUS", "severity": "L2",
        "status": "open", "person_id": "P-001", "device_id": "EXO-001",
    ...
    try:
        ...
        check("系统健康：服务与版本信息", st["services"]["database"] == "healthy"
              and st["model"]["active"] and st["rule_version"] and "急停" in st["safety_boundary"])
        ...
        check("来源切换过滤", len(req(base, "/api/devices?source=real")["items"]) == 0
              and len(req(base, "/api/devices?source=simulated")["items"]) == 3)

        # 3 实时态势：最新遥测+推理，携带来源
        t = req(base, "/api/telemetry?device_id=EXO-001")
        check("实时态势：遥测+推理+来源", t.get("mode") == "realtime" and t.get("inference")
              and t["source_type"] == "simulated" and t.get("source_label"))

        # 4 回放：正序序列，mode=replay
        series = req(base, "/api/telemetry/series?device_id=EXO-001&start=%s&end=%s" % (start, end))
        ts = [x["timestamp"] for x in series["items"]]
        check("数据回放：回放态+时间正序", series["mode"] == "replay" and len(ts) > 5 and ts == sorted(ts))

        # 5 原始数据导出
        exp = req(base, "/api/telemetry/export?device_id=EXO-001&start=%s&end=%s" % (start, end))
        check("原始片段导出", exp["export_type"] == "raw_slice" and exp["slice"]["record_count"] > 0)

        # 6 事件：证据窗 ±30 秒 + 状态处置
        events = req(base, "/api/events")["items"]
        check("事件中心：可控风险事件已生成", len(events) > 0, "%d 条" % len(events))
        if events:
            det = req(base, "/api/event?id=" + events[0]["event_id"])
            check("事件证据窗：±30 秒原始记录", det["evidence_window_sec"] == 30
                  and len(det["evidence_records"]) > 0)
            upd = req(base, "/api/event/status", "POST",
                      {"event_id": events[0]["event_id"], "status": "confirmed", "handled_by": "班组长A"})
            check("事件处置：状态更新", upd["ok"] and upd["event"]["status"] == "confirmed")
            req(base, "/api/event/status", "POST",
                {"event_id": events[0]["event_id"], "status": "open", "handled_by": "班组长A"})

        # 7 任务推荐：硬约束拦截 + 人工确认
        rec = req(base, "/api/tasks/recommend", "POST",
                  {"required_skill": "巡检", "zone_id": "工位1", "load_level": 0.5})
        p1 = next(x for x in rec["items"] if x["person_id"] == "P-001")
        check("硬约束拦截：无技能者不可推荐并注明原因", not p1["eligible"]
              and any("无技能" in b for b in p1["blocked_by"]))
        bad = req(base, "/api/tasks/confirm", "POST",
                  {"required_skill": "巡检", "person_id": "P-001", "confirmer": "班组长A"})
        check("人工确认：被拦截者确认失败", not bad["ok"])
        no_conf = req(base, "/api/tasks/confirm", "POST",
                      {"required_skill": "搬运", "person_id": "P-002", "confirmer": ""})
        check("人工确认：缺确认人拒绝（禁止自动派工）", not no_conf["ok"])
        ok = req(base, "/api/tasks/confirm", "POST",
        ...
        check("助手：最高负荷引用真实证据", not a1["refused"] and a1["evidence"]
              and a1["evidence"][0].get("record_id"))
        ...
        # 9 场景评估：评分 + 一票否决
        ev1 = req(base, "/api/scenario/evaluate", "POST",
                  {"people": 24, "structured": 5, "roi": 4, "payer": 4, "fit": 4,
                   "compliance": 5, "replicate": 4})
        ...
        check("重启后数据恢复", after >= before > 0, "遥测 %d 条" % after)
        storage2.close()
        ...
        check("断网运行：前端无外部 CDN/字体引用", "http://" not in html.replace("http://%s" % "", "")
    ...
    print("\n自检结果：%d 项通过，%d 项失败" % (len(results) - len(failed), len(failed)))
    if failed:
