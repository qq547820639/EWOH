...
try:
    ...
    def encode_telemetry(pitch_deg=0.0, roll_deg=0.0, ax_mg=0, ay_mg=0, az_mg=0,
                         gx_dps=0.0, gy_dps=0.0, gz_dps=0.0, torque_nm=0.0,
                         assist_pct=0, battery_pct=0, seq=0, ts_ms=0):
        payload = struct.pack(
            "<9h",
            _phys_to_i16(pitch_deg, 0.1), _phys_to_i16(roll_deg, 0.1),
            _to_i16(ax_mg), _to_i16(ay_mg), _to_i16(az_mg),
...
TYPE_NAMES = {
    0x01: "HEARTBEAT", 0x02: "IDENT", 0x10: "TELEMETRY",
...
def _type_label(t):
    return "0x%02X %s" % (t, TYPE_NAMES.get(t, "UNKNOWN"))
...
def _build_specs():
    ...
    specs.append((
        "telemetry_normal",
        encode_telemetry(pitch_deg=32.1, roll_deg=2.4, az_mg=9810, gy_dps=18.4,
                         torque_nm=18.6, assist_pct=45, battery_pct=82, seq=100, ts_ms=BASE_TS),
    ...
    corrupted = bytearray(encode_telemetry(pitch_deg=30.0, torque_nm=15.0, battery_pct=80,
                                           seq=200, ts_ms=BASE_TS))
    ...
    gap_raw = b"".join(
        encode_telemetry(pitch_deg=30.0, torque_nm=12.0, battery_pct=80,
                         seq=s, ts_ms=BASE_TS + i * 50)
...
def main():
    ...
    print("fixtures 生成目录: %s" % OUT_DIR)
    print("协议来源: %s" % ("edge_platform.edge.protocol" if _USING_REAL_PROTOCOL else "内置回退实现"))
    print("协议版本: %s" % PROTOCOL_VERSION)
    print("fixtures 数量: %d" % len(entries))
    for e in entries:
        flag = "OK" if e["crc_ok"] else "CRC-FAIL"
        print("  - %-28s %-14s frames=%d bytes=%d [%s]"
              % (e["file"], e["frame_type"], e["frame_count"], e["bytes_len"], flag))
    print("index.json 条目数: %d" % len(entries))
